...
The code is the fabrication kit in `tools/`. Read `tools/API_FABRICATION.md`, not the modules. The worked example is `experiments/16_Expressive_Structure/fabrication/GPT-6/` (README, model, cut list, drawings, pedestal and front page scripts); copy its shape.
...
- Every sheet is portrait, for binding. A view or a composition that is wider than tall is drawn on a landscape blank (`sheets.blank(landscape=True)`), centred with `sheets.place(bounds, sheet)`, and `SheetSet.add` turns it a quarter turn: its bottom edge lies along the right edge and the drawing reads with the sheet turned clockwise, while the labels read upright, because such a blank sets its text at -90 degrees. A label offset written for the sheet as read (a baseline a little below a centre, a name under a strip) goes through `sheet.offset(dx, dy)` so it turns with the page; `lollipop`, `annotate` and `paint_extras` already do. A composed sheet that has to read upright (the pedestal sheets) is laid out portrait from the start.
- Title block, 32 mm, left to right: the QR code to the viewer model; two list columns with DATE and the credits of the set, `SheetSet(..., credits=[('SUPERVISION', ...), ('AGENTS', ...), ('WORKFLOW', ...), ('DISCLAIMER', ...), ('SOURCE CODE', ...)])`, split where the line count balances and spread between the top and bottom of the QR code; SCALE ('1:15, drawn 1:1 to the model, units mm') over a print check bar of 1 m real on the QR code's bottom line, ticks every 10 cm labelled 0cm, 50cm, 100cm; TITLE over PROJECT (`experiment`, 'Experiment 16 - Expressive Structure, GPT-6 v02') over the studio mark in MEK-Mono, flush right; the sheet number in a black box at the right edge, the side seen first in a bound folder. Printing is at 100 %. The layout is `title_blocks.FOOTERS['r']`, the default, chosen by Luka on 2026-09-25 from the study; `SheetSet(..., footer='a')` to `'q'` picks an older one, `title_blocks.HEIGHTS` is what each keeps clear, and `tools/footer_study/pdf/00_all_versions.pdf` shows them all. A note longer than the sheet wraps onto a second line.
- Text pages: a set opens with the run's documents (brief, concept, design notes, version notes) behind a project description, flowed by `tools/newsprint.py` into four justified columns of Times like an old newspaper, at the largest type that fills two pages, cut after the second. `SheetSet.page` adds such a sheet without a title block; it takes a number like any other. See `fab_frontpage.py` of the worked example: the masthead, the QR code to the repository and the agent team organigram (`tools/organigram.py`) are drawn there.
- `SheetSet.write` prints the PDFs with headless Chrome, which embeds the fonts (Segoe UI and Times New Roman from Windows, MEK-Mono from `viewer/fonts`). The SVGs name the fonts without embedding them; `sheet_png.py` adds MEK-Mono when it rasterizes.
...
| Two stacked label lines land side by side on a turned sheet | The offsets were written in landscape mm. Put them through `sheet.offset`. |
