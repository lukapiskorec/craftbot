...
def cli_settings(args, settings):
    ...
    for name in DEFAULTS:
        ...
        if value is not None:
            settings[name] = value
    if settings["samples"] < 1 or min(settings[k] for k in ("edge_width", "fill_strength", "world_strength", "sun_intensity")) < 0:
...
def setup(scene, settings):
    ...
    # Add neutral fill only to diffuse rays: keep the blue camera background and
    # clear-glass transmission/reflections unchanged. There is no ground mesh.
    ...
    if camera_background:
        ...
        links.new(paths.outputs["Is Camera Ray"], camera_mix.inputs[0])
...
def material_role(obj, overrides):
    ...
    for role, pattern in (("glass", r"glass|glazing"), ("concrete", r"concrete|foundation|plinth|footing"),
