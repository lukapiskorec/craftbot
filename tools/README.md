...
| `vector_pdf.py` | Chrome for `print_pdf` | `Sheet` canvas in mm (polygons, lines, circles, styled text with angle, fitted width and word spacing; `turned` for a quarter turn, `turn_labels` and `offset` for labels that read upright after it), SVG output, `print_pdf` (headless Chrome, embeds the fonts a sheet uses), `write_pdf` (standard library, Helvetica only), paper sizes and line weights |
| `title_blocks.py` | | the sheet footers `FOOTERS['a'..'r']`, picked with `SheetSet(..., footer='r')`; `DEFAULT` is `r` (32 mm: QR left, two list columns of the set's credits, a 1 m real scale bar, title over project over studio, number box right for binding); `l` is the 40 mm band version, `m` to `q` other 32 mm trials; `HEIGHTS` is what each keeps clear; `footer_study/footer_study.py` prints them all as strips for comparison |
| `qr_code.py` | | `qr_matrix`: byte mode, level L, versions 1 to 5 (106 bytes) |
| `fonts.py` | Pillow | `advance`: real text widths from the Windows font files and `viewer/fonts`, for justified columns and label boxes; falls back to the Helvetica guess |
| `newsprint.py` | Pillow | markdown flowed into dense justified newspaper columns on a sheet: `parse`, `Newsprint.flow`, `fit` (largest type that fits a page count); the text pages at the front of a set |
| `organigram.py` | | the CraftBot agent team organigram as vector lines, a redrawing of `visuals/craftbot_agent_team_organigram.tex` |
