...
CraftBot spawns you with the experiment id and the run folder name `<Agent>` (the folder under `experiments/NN_*/` named after the model running the experiment: `Fable`, `Opus 5.1`, ...). Pass it to every `closeout.py` call as `--agent "<Agent>"`; the script refuses to guess when an experiment has several run folders.
