...
The default run designs one building. Only when the brief explicitly asks for several variations of the building does the run follow the skill's "Multi-variation runs" section: a shared stage first, then one team per variation in its own run folder.
