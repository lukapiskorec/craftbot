...
for name, node in (("T1", T1), ("T2", T2)):
    # 450 long along the rafter, from the rafter top down 72 + 110 into the web
    d = Vector((CT, ST)) if name == "T1" else Vector((-CT, ST))
    nrm = Vector((-d.y, d.x))
    if nrm.y < 0:
        nrm = -nrm                      # always out of the roof
...
for south in (True, False):
    ...
    z_top = roof_pt(True, v_at(-OVH - FASCIA_T, Z_SHEET0), Z_SHEET0)[1]   # sheet underside over the fascia outer face
    if south:
...
print("experiment_04_fable_v02: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
