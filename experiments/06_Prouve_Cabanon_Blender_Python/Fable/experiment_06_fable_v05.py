...
for tag, xa, xb in (("W", X_ROOF0, XC), ("E", XC, X_ROOF1)):
    ...
    box(f"Ridge_{tag}_EndPost_0", C_RIDGE, xa, xa + DIAG_W, RY0, RY1, ZW0, ZW1)
    box(f"Ridge_{tag}_EndPost_1", C_RIDGE, xb - DIAG_W, xb, RY0, RY1, ZW0, ZW1)
    x, k = xa + DIAG_W + RWEB_GAP / 2, 0
    while x + RWEB_STEP < xb - DIAG_W - RWEB_GAP / 2:
        a = Vector((x, ZW0 if k % 2 == 0 else ZW1))
        b = Vector((x + RWEB_STEP, ZW1 if k % 2 == 0 else ZW0))
        poly = strip(a, b, DIAG_W, ext=0.1)
        for p, nn in (((0, ZW0), (0, 1)), ((0, ZW1), (0, -1)), ((xa + DIAG_W, 0), (1, 0)), ((xb - DIAG_W, 0), (-1, 0))):
            poly = clip(poly, p, nn)
        prism_y(f"Ridge_{tag}_Diag_{k:02d}", C_RIDGE, RY0, RY1, poly)
...
print("experiment_06_fable_v05: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
