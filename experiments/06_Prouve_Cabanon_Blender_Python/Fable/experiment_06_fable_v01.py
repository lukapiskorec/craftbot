...
L = W = 6.0                       # platform / outer wall faces
...
BEARER_Y = [0.05, 1.525, 3.0, 4.475, 5.95]   # bearer centre lines (along X)
FOOT_X = [0.30, 2.10, 3.90, 5.70]

# walls
POST = 0.10                                  # 100 x 100 posts on the module lines
...
FASCIA_T = 0.020
X_ROOF0, X_ROOF1 = -OVH, L + OVH
Y_EAVE = -OVH

# openings
WIN_W, WIN_H, WIN_SILL = 0.70, 1.10, 0.90
...
for j, yc in enumerate(BEARER_Y):
    ...
    box(f"Bearer_{j}", C_FLOOR, 0.0, L, yc - BEARER_W / 2, yc + BEARER_W / 2, Z_FOOT_TOP, Z_BEARER1)
for i, xc in enumerate(positions(0.0, L, 0.60, JOIST_W)):
    box(f"Joist_{i:02d}", C_FLOOR, xc - JOIST_W / 2, xc + JOIST_W / 2, JOIST_W, W - JOIST_W, Z_BEARER1, Z_JOIST1)
box("Rim_Joist_S", C_FLOOR, 0.0, L, 0.0, JOIST_W, Z_BEARER1, Z_JOIST1)
box("Rim_Joist_N", C_FLOOR, 0.0, L, W - JOIST_W, W, Z_BEARER1, Z_JOIST1)
n, y = 0, 0.0
while y < W - 1e-6:
    yy = min(y + BOARD_W, W)
    box(f"Floor_Board_{n:02d}", C_BOARDS, 0.0, L, y, yy, Z_JOIST1, FFL)
    n += 1
    y = yy
...
for side, tag in ((-1, "W"), (1, "E")):
    ...
    clips = [((0, FFL), (0, 1)), ((0, Z_HEAD0), (0, -1)), ((XC, 0), (-side, 0))]
    ...
    # diagonals between the chord inner faces, zigzag, 40 mm apart at the nodes
...
RIDGE_PROFILE = [(YM - RIDGE_W / 2, Z_RIDGE0), (YM + RIDGE_W / 2, Z_RIDGE0),
                 (YM + RIDGE_W / 2, Z_RIDGE1), (YM, z_u(YM)), (YM - RIDGE_W / 2, Z_RIDGE1)]
prism_x("Ridge_Beam_W", C_RIDGE, X_ROOF0, XC, RIDGE_PROFILE)
prism_x("Ridge_Beam_E", C_RIDGE, XC, X_ROOF1, RIDGE_PROFILE)
...
SIDE_POST_X = [0.0] + [k * MOD - POST / 2 for k in range(1, 6)] + [L - POST]
for j, (ya, yb) in enumerate(((0.0, POST), (W - POST, W))):
    for i, xa in enumerate(SIDE_POST_X):
        box(f"Post_Side{j}_{i}", C_POSTS, xa, xa + POST, ya, yb, FFL, Z_POST1)
...
for end, (xa, xb) in enumerate(((0.0, POST), (L - POST, L))):
    for i, yc in enumerate(END_POST_Y):
        ya, yb = yc - POST / 2, yc + POST / 2
        prism_x(f"Post_End{end}_{i}", C_POSTS, xa, xb, [(ya, FFL), (yb, FFL), (yb, z_u(yb)), (ya, z_u(ya))])
...
SIDE_BAYS = []
for i in range(6):
    SIDE_BAYS.append((SIDE_POST_X[i] + POST, SIDE_POST_X[i + 1]))
...
END_POSTS = sorted([(0.0, POST), (W - POST, W)] +
                   [(yc - POST / 2, yc + POST / 2) for yc in END_POST_Y + [GY0, GY1]])
...
for south in (True, False):
    ...
    for k in range(n_trays):
        ...
        prism_x(f"Tray_{tag}_{k:02d}_Deck", C_TRAY, xa, xb, tray_profile(south, Y_EAVE, YM, RIB_D, RIB_D + DECK_T))
...
box("Step_0", C_STAIR, -0.60, 0.0, YM - 0.6, YM + 0.6, 0.0, FFL / 2)
box("Step_1", C_STAIR, -0.30, 0.0, YM - 0.6, YM + 0.6, FFL / 2, FFL)

print("experiment_06_fable_v01: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
