# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 04 - CHATGPT 5.1 - V08
...
def build_walls(floor_z):
    ...
    # ------------------------------------------------------------------
    # HORIZONTAL MEMBERS SIZES / POSITIONS (lintel, sill, head)
    # Make vertical depth = STUD_WIDTH so we can trim studs cleanly.
    # ------------------------------------------------------------------

    h_depth = STUD_WIDTH      # vertical depth of lintels / sills / heads
    h_thick = STUD_WIDTH      # plan thickness

    # Door lintel centre/bounds
    door_lintel_center_z = door_top_z + half(h_depth)
    door_lintel_bottom_z = door_top_z
    door_lintel_top_z = door_top_z + h_depth

    # Window sill centre/bounds
    win_sill_center_z = win_sill_z - half(h_depth)
    win_sill_bottom_z = win_sill_z - h_depth
    win_sill_top_z = win_sill_z

    # Window head centre/bounds
    win_head_center_z = win_head_z + half(h_depth)
    win_head_bottom_z = win_head_z
    win_head_top_z = win_head_z + h_depth

    # Clear opening extents for STUD trimming (include horizontals):
    ...
    # ------------------------------------------------------------------
    # STUDS WITH TRIMMED SEGMENTS (jamb studs remain, but notched out)
    # ------------------------------------------------------------------
    ...
    for i, x in enumerate(stud_x_positions):
        open_ranges = []
        # door clear opening (including lintel region)
        if door_clear_x0 < x < door_clear_x1:
            open_ranges.append((door_clear_z0, door_clear_z1))
        # south window clear opening (including sill+head depth)
    ...
    # ------------------------------------------------------------------
    # LINTELS, WINDOW SILLS / HEADS (now use h_depth vertically)
    # ------------------------------------------------------------------
    ...
    # Openings data per wall for plywood (use rough opening heights)
    openings_s = [
...
def build_roof(wall_top_z):
    ...
    # Ceiling / roof beams: copy floor joist layout (along Y),
    # but extend beyond the centre-lines of Top_S / Top_N beams.
    ...
    add_slab("Roof", x0, x1, y_s, y_n, roof_center_z, ROOF_THICK)
