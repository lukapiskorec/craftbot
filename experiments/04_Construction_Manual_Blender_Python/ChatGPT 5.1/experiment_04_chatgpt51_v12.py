# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 04 - CHATGPT 5.1 - V12
...
def build_truss_webs_king_post(
    ...
    # centre bottom and rafters midpoints
    y_center = y_mid
...
def build_truss_webs_fink(
    ...
    """
    Internal members for a Fink truss (W-shaped).
    Approximate layout:
    - bottom chord nodes: left, centre, right
    - upper nodes: midpoints of each rafter
    - diagonals form a W pattern.
    """
    y_center = y_mid
    y_left = y_s
    y_right = y_n

    # rafter midpoints (upper nodes)
    y_mid_left = 0.5 * (y_s + y_mid)
    y_mid_right = 0.5 * (y_n + y_mid)
    z_mid_left = z_on_rafter(y_mid_left, y_s, y_n, wall_top_z, roof_rise)
    z_mid_right = z_on_rafter(y_mid_right, y_s, y_n, wall_top_z, roof_rise)

    # Optional small vertical at centre to split diagonals (short king post)
    z_stub_top = (chord_top_z + ridge_z) * 0.5
    add_vertical_member(
        f"Fink_Stub_{j}",
        x,
        y_center,
        chord_top_z,
        z_stub_top,
        width=JOIST_WIDTH * 0.7,
    )

    # Diagonals forming W:
    # left support -> left upper node
    add_diagonal_member(
        f"Fink_Left_Lower_{j}",
        x,
        y_left,
        chord_top_z,
        y_mid_left,
        z_mid_left,
        width=JOIST_WIDTH * 0.8,
    )
    # centre bottom -> left upper node
    add_diagonal_member(
        f"Fink_Left_Upper_{j}",
        x,
        y_center,
        chord_top_z,
        y_mid_left,
    ...
    # centre bottom -> right upper node
    add_diagonal_member(
        f"Fink_Right_Upper_{j}",
        x,
        y_center,
        chord_top_z,
        y_mid_right,
    ...
    # right support -> right upper node
    add_diagonal_member(
        f"Fink_Right_Lower_{j}",
        x,
        y_right,
        chord_top_z,
        y_mid_right,
