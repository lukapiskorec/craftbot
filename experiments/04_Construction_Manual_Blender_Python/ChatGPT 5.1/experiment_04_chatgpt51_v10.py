# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 04 - CHATGPT 5.1 - V10
...
def add_roof_plane(name, x0, x1, y_low, y_mid, wall_top_z, rise, thick):
    ...
    Bottom face is aligned with the top of the rafters,
    so the plane sits ON TOP of them, not through their centre.
    """
    span_y = abs(y_mid - y_low)
    if span_y <= EPS:
        return
    ...
    # vertical offset to move roof plane on top of rafters:
    # + half rafter depth and + half roof thickness (projected vertically)
    rafter_center_z = wall_top_z + rise * 0.5
    vertical_offset = half(JOIST_DEPTH) * math.cos(theta) + half(thick) * math.cos(theta)
    zc = rafter_center_z + vertical_offset
...
def add_vertical_member(name, x, y, z0, z1, width, depth):
...
def add_diagonal_member(name, x, y0, z0, y1, z1, width, depth):
    """Diagonal web member in the truss plane (XZ)."""
    dy = y1 - y0
    dz = z1 - z0
...
def build_roof(wall_top_z):
    ...
    # ------------------ Trussed rafters (top chords + webs) -------------
    y_mid = 0.5 * (y_s + y_n)
    span_half = y_mid - y_s
    ...
    for j in range(n_beams):
        ...
        # --- Truss webs: king post + two diagonals ---

        # King post in the centre between bottom chord and ridge
        y_mid_chord = y_mid
        add_vertical_member(
            f"KingPost_{j}",
            x,
        ...
        # Diagonal webs from bottom chord quarters to king post
        y_q_s = 0.5 * (y_s + y_mid)
        y_q_n = 0.5 * (y_n + y_mid)
        z_bottom = chord_top_z
        z_top = 0.5 * (chord_top_z + ridge_z)  # meet king post about mid-height
