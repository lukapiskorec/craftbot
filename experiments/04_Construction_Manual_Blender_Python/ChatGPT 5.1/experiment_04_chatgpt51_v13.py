# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 04 - CHATGPT 5.1 - V13
...
def add_roof_plane(name, x0, x1, y_low, y_mid, wall_top_z, rise, thick):
    ...
    Bottom face is aligned with (and slightly above) top of rafters.
    """
    span_y = abs(y_mid - y_low)
    if span_y <= EPS:
        return
    ...
    # rafters: centre at wall_top_z + rise * 0.5
    rafter_center_z = wall_top_z + rise * 0.5
    # move plane so its *bottom* sits on top of rafter, plus tiny clearance
...
def add_diagonal_member(name, x, y0, z0, y1, z1, width):
    ...
    craftbot.place_element(
        ...
        axis=(1.0, 0.0, 0.0),
        angle=angle_deg,
        scale=(half(width), half(L), half(width)),
    )


def z_on_rafter(y, y_s, y_n, wall_top_z, roof_rise):
    """
    Given a Y coordinate between the eaves, return the Z on the rafter line
...
def build_truss_webs_king_post(
    ...
    """Internal members for a king-post truss."""
    y_center = y_mid
...
def build_truss_webs_fink(
    ...
    """
    Internal members for a Fink truss (W-shaped), using nodes:
    - bottom chord: left support (y_s), mid-span (y_mid), right support (y_n)
    - rafters: intermediate nodes at quarter-span positions
    - apex at ridge_z above y_mid
    Web layout (per reference diagram):
      B_left  -> R_left
      B_mid   -> R_left
      B_mid   -> R_right
      B_right -> R_right
    ...
    # positions of intermediate rafter nodes (approx quarter-span)
    y_r_left = 0.5 * (y_left + y_center)
    y_r_right = 0.5 * (y_center + y_right)

    z_r_left = z_on_rafter(y_r_left, y_s, y_n, wall_top_z, roof_rise)
    z_r_right = z_on_rafter(y_r_right, y_s, y_n, wall_top_z, roof_rise)

    # Outer webs: supports -> rafter nodes
    add_diagonal_member(
        f"Fink_Left_Outer_{j}",
        x,
        y_left,
        chord_top_z,
        y_r_left,
    ...
    # Inner webs: bottom centre -> rafter nodes
    add_diagonal_member(
        f"Fink_Left_Inner_{j}",
        x,
        y_center,
        chord_top_z,
        y_r_left,
...
def build_roof(wall_top_z):
    ...
    for j in range(n_beams):
        ...
        # Rafters from wall plate to ridge
        add_rafter(
            f"Rafter_S_{j}",
            x,
            y_s,
        ...
        # Web system depending on chosen truss type
