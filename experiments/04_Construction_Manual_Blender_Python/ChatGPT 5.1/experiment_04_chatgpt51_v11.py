# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 04 - CHATGPT 5.1 - V11
...
# ---------------------------------------------------------------------------
# ROOF: CEILING JOISTS + KING-POST TRUSS + ROOF SHEETS
# ---------------------------------------------------------------------------
...
def add_roof_plane(name, x0, x1, y_low, y_mid, wall_top_z, rise, thick):
    ...
    Bottom face is aligned with top of rafters.
    """
    span_y = abs(y_mid - y_low)
    if span_y <= EPS:
        return
...
def z_on_rafter(y, y_s, y_n, wall_top_z, rise):
    """
    ...
    y_mid = 0.5 * (y_s + y_n)
    span_half = y_mid - y_s
    theta = math.atan2(rise, span_half)
    if y <= y_mid:
        dist = y - y_s
    else:
        dist = y_n - y
...
def build_roof(wall_top_z):
    ...
    # ------------------ King-post trusses (top chords + webs) ----------
    y_mid = 0.5 * (y_s + y_n)
    span_half = y_mid - y_s
    ...
    for j in range(n_beams):
        ...
        # --- King post (centre bottom chord to ridge) ---
        y_center = y_mid
        add_vertical_member(
            f"KingPost_{j}",
            x,
        ...
        # --- Diagonal struts from bottom-chord centre to rafters midspan ---

        # midpoints along rafters (in plan)
        ...
        # diagonals from bottom-centre to those rafter midpoints
        add_diagonal_member(
            f"Strut_Left_{j}",
            x,
            y_center,
