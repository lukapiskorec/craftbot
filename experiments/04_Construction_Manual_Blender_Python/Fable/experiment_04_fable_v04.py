...
# ------------------------------------------------------------------
# v04: truss bottom chord ends 28 mm past the wall face (manual truss
# detail) instead of at the platform edge; heel, eaves and gable measured
# from that point.
...
CH0, CH1 = Y0 - 0.028, Y1 + 0.028 # truss bottom chord ends (heel bearing + 28, manual truss detail)
...
TR_SP = 1.22
RISE = 1.195
OVH = 0.86 - 0.028                # rafter tail 860 from the wall face, measured from the chord end
GABLE_OVH = 0.30
GUSSET = 0.009
GABLE_PLY = 0.006
...
def zu_s(y):
    return ZB1 + SLOPE * (y - CH0)


def zu_n(y):
    return ZB1 + SLOPE * (CH1 - y)


BELOW_S = ((CH0, ZB1), (SLOPE, -1.0))
BELOW_N = ((CH1, ZB1), (-SLOPE, -1.0))
ABOVE_CHORD = ((CH0, ZB1), (0.0, 1.0))
BELOW_S_TOP = ((CH0, ZB1 + DV), (SLOPE, -1.0))
BELOW_N_TOP = ((CH1, ZB1 + DV), (-SLOPE, -1.0))
ABOVE_CHORD_BOT = ((CH0, ZB0), (0.0, 1.0))

B1, B2 = (CH0 + (CH1 - CH0) / 3, ZB1), (CH0 + 2 * (CH1 - CH0) / 3, ZB1)
T1, T2 = ((CH0 + YM) / 2, zu_s((CH0 + YM) / 2)), ((CH1 + YM) / 2, zu_n((CH1 + YM) / 2))
APEX = (YM, zu_s(YM))
RAFTER_S = [(CH0 - OVH, zu_s(CH0 - OVH)), (YM, zu_s(YM)), (YM, zu_s(YM) + DV), (CH0 - OVH, zu_s(CH0 - OVH) + DV)]
RAFTER_N = [(CH1 + OVH, zu_n(CH1 + OVH)), (YM, zu_n(YM)), (YM, zu_n(YM) + DV), (CH1 + OVH, zu_n(CH1 + OVH) + DV)]
WEBS = [
    ("W1", B1, T1, [((B1[0], 0), (-1, 0))]),
...
GUSSETS = [
    ...
    ("HeelS", (CH0, CH0 + 0.15), (ZB0, ZB0 + 0.10)),
    ("HeelN", (CH1 - 0.15, CH1), (ZB0, ZB0 + 0.10)),
...
for i, x0 in enumerate(truss_x):
    x1 = x0 + TR_T
    t = f"Truss_{i}"
    prism_x(f"{t}_BottomChord", C_ROOF, x0, x1, [(CH0, ZB0), (CH1, ZB0), (CH1, ZB1), (CH0, ZB1)])
...
def roof_frame(south):
    if south:
        return (0.0, CH0, ZB1), (1, 0, 0), (0, CT, ST)
    return (PL, CH1, ZB1), (-1, 0, 0), (0, -CT, ST)
...
for south in (True, False):
    ...
    for end in (0, 1):
        ...
        if not south:
            ua, ub = PL - ub, PL - ua
        va, vb = y_to_v(YM - 0.15), y_to_v(CH0 + 0.15)
...
for name, a, b in (("W1", B1, T1), ("W4", B2, T2)):
    ...
    d = (b - a).normalized()
    nrm = Vector((-d.y, d.x))
    if nrm.dot(Vector((CH0 if name == "W1" else CH1, ZB1)) - mid) < 0:
        nrm = -nrm
...
def v_at(y, t):
    return (y - CH0 + t * ST) / CT


Y_TAIL = CH0 - OVH
V_TAIL = v_at(Y_TAIL, 0.0)
V_APEX = v_at(YM, 0.0)
...
for south in (True, False):
    ...
    z_top = roof_pt(True, v_at(Y_TAIL - FASCIA_T, Z_SHEET0), Z_SHEET0)[1]
    if south:
        box("Fascia_S", C_COVER, X_ROOF0 - FASCIA_T, X_ROOF1 + FASCIA_T, Y_TAIL - FASCIA_T, Y_TAIL, z_top - FASCIA_D, z_top)
    else:
        box("Fascia_N", C_COVER, X_ROOF0 - FASCIA_T, X_ROOF1 + FASCIA_T, PW - Y_TAIL, PW - Y_TAIL + FASCIA_T, z_top - FASCIA_D, z_top)
    for end, (xa, xb) in enumerate(((X_ROOF0 - FASCIA_T, X_ROOF0), (X_ROOF1, X_ROOF1 + FASCIA_T))):
        pts = [roof_pt(south, v_at(Y_TAIL, Z_SHEET0 - FASCIA_D), Z_SHEET0 - FASCIA_D),
               roof_pt(south, v_at(YM, Z_SHEET0 - FASCIA_D), Z_SHEET0 - FASCIA_D),
               roof_pt(south, v_at(YM, Z_SHEET0), Z_SHEET0),
               roof_pt(south, v_at(Y_TAIL, Z_SHEET0), Z_SHEET0)]
        prism_x(f"Barge_{tag}_{end}", C_COVER, xa, xb, pts)

# ------------------------------------------------------------------
# 9. GABLE ENDS: 6 mm ply on the outer face of the end trusses

GABLE_MAIN = [(CH0, ZB0), (CH1, ZB0), (CH1, zu_n(CH1) + DV), (YM, zu_s(YM) + DV), (CH0, zu_s(CH0) + DV)]
GABLE_TAIL_S = [(Y_TAIL, zu_s(Y_TAIL)), (CH0, zu_s(CH0)), (CH0, zu_s(CH0) + DV), (Y_TAIL, zu_s(Y_TAIL) + DV)]
GABLE_TAIL_N = [(PW - Y_TAIL, zu_n(PW - Y_TAIL)), (CH1, zu_n(CH1)), (CH1, zu_n(CH1) + DV), (PW - Y_TAIL, zu_n(PW - Y_TAIL) + DV)]
...
print("experiment_04_fable_v04: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
