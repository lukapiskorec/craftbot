...
def stud_wall(prefix, coll, along, a0, a1, b0, b1, z_sole, z_top, openings=()):
    """Stud wall with sole plate, studs @600 and double top plate.
    along = 'x' or 'y' is the wall direction; a0..a1 its extent along that
    axis, b0..b1 its thickness across.  openings = [(aa, ab, z_head)] door
    openings framed with jack + king studs, a 150 mm header and cripples."""
    ...
    for i, c in enumerate(positions(a0, a1, SPACING, PLATE)):
        if any(aa - 2 * PLATE - PLATE / 2 < c < ab + 2 * PLATE + PLATE / 2 for aa, ab, _ in openings):
            continue
        rect(f"{prefix}_Stud_{i:02d}", c - PLATE / 2, c + PLATE / 2, z0, z1)
    for k, (aa, ab, zh) in enumerate(openings):
...
def loft_box(prefix, x0, x1):
    ...
    tile_wall_x(f"{prefix}_Clad_S", C_SHE, x0, x1, y0 - PLY, y0, Z_ROOF_DECK, Z_BOX_TOP)
    tile_wall_x(f"{prefix}_Clad_N", C_SHE, x0, x1, y1, y1 + PLY, Z_ROOF_DECK, Z_BOX_TOP)


loft_box("Box1", *BOX1_X)
loft_box("Box2", *BOX2_X)
...
print("VIPP shelter v01 generated:", len([o for o in bpy.data.objects if o.type == "MESH"]), "elements")
