...
for i, xc in enumerate(positions(0.0, L, SPACING, JOIST_W)):
    ...
    floor_joists.append((xc - JOIST_W / 2, xc + JOIST_W / 2))
# solid blocking between joists over each girder (stops joist rollover)
for j, y in enumerate(GIRDER_Y):
    for i in range(len(floor_joists) - 1):
        xa, xb = floor_joists[i][1], floor_joists[i + 1][0]
        box(f"Floor_Block_{j}_{i:02d}", C_FLOOR, xa, xb, y - PLATE / 2, y + PLATE / 2, Z_JOIST, Z_SUBFLOOR)
...
stud_wall("Fireplace_Wall_X", C_INT, "x", STOR_X + INT_STUD_D + PLY, 4.50, 1.60, 1.60 + INT_STUD_D, FFL, Z_INT_TOP)
# plywood shear panels on the partitions: with glass on both long sides these
# walls (plus the roof deck diaphragm) are what resists wind along X
C_SHEAR = "Structure/Shear_Panels"
clad_x("Shear_Bath_X", C_SHEAR, STUD_D, BATH_X + INT_STUD_D + PLY, BATH_Y + INT_STUD_D, BATH_Y + INT_STUD_D + PLY,
       FFL, Z_INT_TOP, holes=[(0.40, 1.25, FFL, FFL + 2.10)])
tile_wall_y("Shear_Bath_Y", C_SHEAR, BATH_X + INT_STUD_D, BATH_X + INT_STUD_D + PLY, STUD_D, BATH_Y + INT_STUD_D, FFL, Z_INT_TOP)
tile_wall_y("Shear_Storage_Y", C_SHEAR, STOR_X + INT_STUD_D, STOR_X + INT_STUD_D + PLY, STUD_D, BATH_Y + INT_STUD_D, FFL, Z_INT_TOP)
...
RJ = JOIST_W
# (x0, plies) of the built-up trimmer joists under the box side walls; box 2
# (open light well) gets 3-ply trimmers because they also carry the header
TRIMMERS = [(BOX1_X[0], 2), (BOX1_X[1] - 2 * RJ, 2), (BOX2_X[0] - RJ, 3), (BOX2_X[1] - 2 * RJ, 3)]
roof_joists = []   # (x0, x1, y1) of every member running across, for blocking
for i, (x0, plies) in enumerate(TRIMMERS):
    for k in range(plies):
        box(f"Roof_Trimmer_{i}{'abc'[k]}", C_ROOF, x0 + k * RJ, x0 + (k + 1) * RJ, 0.0, W, Z_RING_TOP, Z_ROOF_JOIST_TOP)
    roof_joists.append((x0, x0 + plies * RJ, W))
HEADER_PLIES = 3
WELL_Y1 = BOX_Y0 + HEADER_PLIES * RJ
# joist grid: 600 c/c in general, 400 c/c under the loft-bed floor (box 1,
# between its trimmers).  The ladder hatch spans two 400 bays: the joist in
# the middle is cut into two tail joists bearing on the hatch headers.
box1_grid = positions(BOX1_X[0] + 2 * RJ, BOX1_X[1] - 2 * RJ, 0.4, RJ)[1:-1]
HATCH_X = (box1_grid[5] + RJ / 2, box1_grid[7] - RJ / 2)      # between joists at x = 3.0 and 3.8
HATCH_Y = (3.40, 4.60)
grid = [x for x in positions(0.0, L, SPACING, RJ) if not BOX1_X[0] < x < BOX1_X[1]] + box1_grid
for i, xc in enumerate(sorted(grid)):
    if any(x0 - RJ < xc < x0 + (plies + 1) * RJ for x0, plies in TRIMMERS):
        continue
    x0, x1 = xc - RJ / 2, xc + RJ / 2
    in_well = BOX2_X[0] + 2 * RJ < xc < BOX2_X[1] - 2 * RJ
    y1 = BOX_Y0 if in_well else W
    if HATCH_X[0] < xc < HATCH_X[1]:
        box(f"Roof_Joist_{i:02d}_tail_S", C_ROOF, x0, x1, 0.0, HATCH_Y[0] - RJ, Z_RING_TOP, Z_ROOF_JOIST_TOP)
        box(f"Roof_Joist_{i:02d}_tail_N", C_ROOF, x0, x1, HATCH_Y[1] + RJ, W, Z_RING_TOP, Z_ROOF_JOIST_TOP)
    else:
        box(f"Roof_Joist_{i:02d}", C_ROOF, x0, x1, 0.0, y1, Z_RING_TOP, Z_ROOF_JOIST_TOP)
    roof_joists.append((x0, x1, y1))
roof_joists.sort()
# light-well header (3-ply) between the box-2 trimmers, directly under the box front wall
for k in range(HEADER_PLIES):
    box(f"Well_Header_{'abc'[k]}", C_ROOF, BOX2_X[0] + 2 * RJ, BOX2_X[1] - 2 * RJ,
        BOX_Y0 + k * RJ, BOX_Y0 + (k + 1) * RJ, Z_RING_TOP, Z_ROOF_JOIST_TOP)
# doubled rim joist on the back ring beam under the box-2 back wall
box("Well_Rim_N_a", C_ROOF, BOX2_X[0] + 2 * RJ, BOX2_X[1] - 2 * RJ, W - 2 * RJ, W - RJ, Z_RING_TOP, Z_ROOF_JOIST_TOP)
box("Well_Rim_N_b", C_ROOF, BOX2_X[0] + 2 * RJ, BOX2_X[1] - 2 * RJ, W - RJ, W, Z_RING_TOP, Z_ROOF_JOIST_TOP)
...
# solid blocking between joist ends over both ring beams (rim blocking);
# on the back beam only where the joists reach it (the well rim covers the rest)
for i in range(len(roof_joists) - 1):
    (_, xa, ya), (xb, _, yb) = roof_joists[i], roof_joists[i + 1]
    if xb - xa < 0.05:
        continue
    box(f"Roof_Block_S_{i:02d}", C_ROOF, xa, xb, 0.0, RJ, Z_RING_TOP, Z_ROOF_JOIST_TOP)
    if ya == W and yb == W:
        box(f"Roof_Block_N_{i:02d}", C_ROOF, xa, xb, W - RJ, W, Z_RING_TOP, Z_ROOF_JOIST_TOP)

C_RDECK = "Floors/Roof_Deck"
well_x0, well_x1 = BOX2_X[0] + BOX_STUD_D, BOX2_X[1] - BOX_STUD_D
well_y0, well_y1 = WELL_Y1, W - BOX_STUD_D
deck_rects = [
...
def loft_box(prefix, x0, x1, windows=()):
    ...
    for k, (xa, xb, zs, zh) in enumerate(windows):
        xm = (xa + xb) / 2
        box(f"{prefix}_Window_{k}_Mullion", C_BOX, xm - PLATE / 2, xm + PLATE / 2, y0, y0 + d, zs, zh)
        box(f"{prefix}_Window_{k}_L", C_GLASS, xa, xm - PLATE / 2, y0 + d / 2 - GLASS / 2, y0 + d / 2 + GLASS / 2, zs, zh)
        box(f"{prefix}_Window_{k}_R", C_GLASS, xm + PLATE / 2, xb, y0 + d / 2 - GLASS / 2, y0 + d / 2 + GLASS / 2, zs, zh)
...
print("VIPP shelter v04 generated:", len([o for o in bpy.data.objects if o.type == "MESH"]), "elements")
