# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 03 - CHATGPT 5.1 - V08
...
# (Loft parameters kept for reference but not used any more)
TOP_STRIP_WIDTH  = 2.5
TOP_STRIP_LENGTH = 10.0
LOFT_LENGTH      = 5.0
...
# ------------------------------------------------------------------
# SKYLIGHTS  (two equal, nearly half length & width, aligned to +Y)
...
# Shifted toward positive-Y facade (aligned to that side)
SKY_EDGE_MARGIN = 0.15  # small margin from outer shell
SKY_CENTER_Y    = UNIT_WIDTH / 2.0 - SKY_WIDTH / 2.0 - SKY_EDGE_MARGIN

# Upstand (skylight wall) height above main roof
# > half the ground-floor height (3.0 m / 2 = 1.5 m)
UPSTAND_HEIGHT = 1.7

# Side panel thickness for skylight walls (cladding)
SKY_PANEL_THICK = 0.04
...
# ------------------------------------------------------------------
# SKYLIGHT UPSTAND HELPER (closed sides, open top, with corner studs)
...
def build_skylight_upstand(prefix, center_x, center_y,
    ...
    Four sides are closed with thin panels; top remains open.
    """
    ...
    # Side panels (cladding) – four sides, top open
    panel_z_center = z_roof_top + wall_height / 2.0

    # Long panels (north/south)
    make_box(
        f"{prefix}_Panel_Long_PosY",
        length_x,
        SKY_PANEL_THICK,
        wall_height,
        (center_x, center_y + length_y / 2.0 - SKY_PANEL_THICK / 2.0, panel_z_center),
    )
    make_box(
        f"{prefix}_Panel_Long_NegY",
        length_x,
        SKY_PANEL_THICK,
        wall_height,
        (center_x, center_y - length_y / 2.0 + SKY_PANEL_THICK / 2.0, panel_z_center),
    ...
    # Short panels (east/west)
    make_box(
        f"{prefix}_Panel_Short_PosX",
        SKY_PANEL_THICK,
        length_y,
        wall_height,
        (center_x + length_x / 2.0 - SKY_PANEL_THICK / 2.0, center_y, panel_z_center),
    )
    make_box(
        f"{prefix}_Panel_Short_NegX",
        SKY_PANEL_THICK,
        length_y,
        wall_height,
        (center_x - length_x / 2.0 + SKY_PANEL_THICK / 2.0, center_y, panel_z_center),
    )
    # No glass element – top remains open
...
def build_roof_framing():
    ...
    # Two identical skylight upstands, aligned toward +Y side
...
def build_shelter():
    ...
    build_floor_framing()
    build_wall_framing()
    build_roof_framing()      # includes skylights
