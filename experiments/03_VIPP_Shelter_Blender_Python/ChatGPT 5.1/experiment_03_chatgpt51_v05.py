# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 03 - CHATGPT 5.1 - V05
...
# Both skylights are centered in the 2.5 m strip in Y:
SKY_CENTER_Y = 0.0
...
def build_roof_framing():
    ...
    def in_skylight_region(x):
        half1 = SKY_MAIN_LENGTH / 2.0
        half2 = SKY_LOFT_LENGTH / 2.0
        in_main = (SKY_MAIN_CENTER_X - half1) <= x <= (SKY_MAIN_CENTER_X + half1)
        in_loft = (SKY_LOFT_CENTER_X - half2) <= x <= (SKY_LOFT_CENTER_X + half2)
...
# ------------------------------------------------------------------
# SIMPLE EXTERIOR SHELL (kept as before)
