...
def wall_pieces(poly, openings):
    ...
    (u0, u1, z0, z1), non-overlapping in u, into convex pieces:
    full-height piers between openings, sill and lintel pieces within
    each opening band."""
    pieces = []
    cur = None
    for u0, u1, z0, z1 in sorted(openings):
        band = clip(poly, (u0, 0), (1, 0)) if cur is None else clip(poly, (cur, 0), (1, 0))
        pieces.append(clip(band, (u0, 0), (-1, 0)))               # pier left of the opening
        col = clip(clip(poly, (u0, 0), (1, 0)), (u1, 0), (-1, 0))  # opening column
        pieces.append(clip(col, (0, z0), (0, -1)))                 # sill
        pieces.append(clip(col, (0, z1), (0, 1)))                  # lintel
        cur = u1
...
def flight(prefix, coll, x_start, direction, y0, y1, z_base, n_risers):
    """n_risers - 1 solid tread blocks from z_base; the last riser lands
    on the landing.  direction = +1 rises towards +X."""
    for i in range(n_risers - 1):
        xa = x_start + direction * GOING * i
        xb = xa + direction * GOING
        top = z_base + RISER * (i + 1)
        box(f"{prefix}_{i + 1:02d}", coll, min(xa, xb), max(xa, xb), y0, y1, top - STEP_D, top)
...
def stair_storey(k, z0, n_risers, land_w, coll_s, coll_l):
    ...
    flight(f"Flight_A_{k}", coll_s, x_a, +1, Y_FA0, Y_FA1, z0, n_risers)
...
for k in range(1, 9):
    ...
    if k <= N_REG:
        ...
        wall_along_y(f"Parting_S_L{k}", C_INT, rect(T_EXT, Y_C0, z, z_top), X_PW - T_INT / 2, X_PW + T_INT / 2)
        wall_along_y(f"Parting_N_L{k}", C_INT, rect(Y_C3, LY - T_EXT, z, z_top), X_PW - T_INT / 2, X_PW + T_INT / 2)
        for tag, y0, y1 in (("S", Y_C0, Y_C1), ("N", Y_C2, Y_C3)):
            for j, (xa, xb) in enumerate(((T_EXT, X_K0), (X_K1, X_PW), (X_PW, LX - T_EXT))):
        ...
        for tag, x0, x1 in (("W", 0, T_EXT), ("E", LX - T_EXT, LX)):
            ...
            ops = [o for o in win[tag] if o[0] >= Y_KN and o[1] <= LY - Y_KN]
        ...
        for tag, y0 in (("S", Y_KN), ("N", LY - Y_KN - T_EXT)):
            for j, (xa, xb) in enumerate(((T_EXT, X_K0), (X_K0, X_K1), (X_K1, LX - T_EXT))):
        ...
        wall_along_y(f"Parting_S_L7", C_INT, below_roof(rect(T_EXT, Y_C0, z, z_top)), X_PW - T_INT / 2, X_PW + T_INT / 2)
        wall_along_y(f"Parting_N_L7", C_INT, below_roof(rect(Y_C3, LY - T_EXT, z, z_top)), X_PW - T_INT / 2, X_PW + T_INT / 2)
        for tag, y0, y1 in (("S", Y_C0, Y_C1), ("N", Y_C2, Y_C3)):
            for j, (xa, xb) in enumerate(((T_EXT, X_K0), (X_K1, X_PW), (X_PW, LX - T_EXT))):
                ops = [o for o in door_ops[tag] if o[0] >= xa and o[1] <= xb]
        ...
        wall_along_y(f"Parting_S_L8", C_INT, below_roof(rect(Y_KN, Y_C0, z, z + 9.0)), X_PW - T_INT / 2, X_PW + T_INT / 2)
        wall_along_y(f"Parting_N_L8", C_INT, below_roof(rect(Y_C3, LY - Y_KN, z, z + 9.0)), X_PW - T_INT / 2, X_PW + T_INT / 2)
        for tag, y0, y1 in (("S", Y_C0, Y_C1), ("N", Y_C2, Y_C3)):
            zt = min(z_roof(y0), z_roof(y1))
            for j, (xa, xb) in enumerate(((T_EXT, X_K0), (X_K1, X_PW), (X_PW, LX - T_EXT))):
                ops = [o for o in door_ops[tag] if o[0] >= xa and o[1] <= xb]
                wall_along_x(f"Corridor_{tag}_L8_P{j + 1}", C_INT, rect(xa, xb, z, zt), y0, y1, ops)
            prism_x(f"Corridor_{tag}_L8_Wedge", C_INT, T_EXT, LX - T_EXT,
                    below_roof(rect(y0, y1, zt, zt + 1.0)))
            for o in door_ops[tag]:
...
for i, (xa, xb) in enumerate(tile_x(ROOF_X0, ROOF_X1)):
    prism_x(f"Roof_S_{i + 1:02d}", C_ROOF, xa, xb, slope_profile(True, 0, T_ROOF, -OVH_E, Y_RIDGE))
    prism_x(f"Roof_N_{i + 1:02d}", C_ROOF, xa, xb, slope_profile(False, 0, T_ROOF, Y_RIDGE, LY + OVH_E))
prism_x("Roof_Covering_S", C_COV, ROOF_X0, ROOF_X1, slope_profile(True, T_ROOF, T_ROOF + T_COV, -OVH_E, Y_RIDGE))
prism_x("Roof_Covering_N", C_COV, ROOF_X0, ROOF_X1, slope_profile(False, T_ROOF, T_ROOF + T_COV, Y_RIDGE, LY + OVH_E))
