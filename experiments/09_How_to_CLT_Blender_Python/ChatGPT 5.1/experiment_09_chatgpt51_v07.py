# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 09 - CHATGPT 5.1 - V07
# HOW TO CLT
# ------------------------------------------------------------------
...
STAIR_LANDING_LENGTH   = 1.00      # mid-landing depth (along Y)
...
STAIR_CLEAR_ELEV_X     = 0.10      # clearance between elevators and stair

# Elevator
ELEVATOR_CLEAR_TOP     = 0.0       # extra freeboard, can be 0
...
def core_interior_bounds():
    """
    Inside faces of the core CLT walls, used for elevator, stair
    and internal slabs.
    """
...
def elevator_bounds():
    """
    X/Y bounds for the WEST elevator zone, leaving a gap to the stair
    towards +X. Elevators fill that west side in plan.
    """
    x_in_min, x_in_max, y_in_min, y_in_max = core_interior_bounds()
    x_min_e = x_in_min
    x_max_e = 0.0 - STAIR_CLEAR_ELEV_X
    return x_min_e, x_max_e, y_in_min, y_in_max


def stair_layout_params():
    """
    Compute origin and extents for the double-flight stair positioned
    along the +X edge of the core.
    """
    x_in_min, x_in_max, y_in_min, y_in_max = core_interior_bounds()

    double_width = 2.0 * STAIR_WIDTH + STAIR_LANDING_GAP

    # centre of both flights/landing, hugging +X interior wall
    ox = x_in_max - STAIR_CLEAR_CORE_Y - double_width / 2.0

    # Y-position so the whole stair fits inside the core
    visible_steps_per_flight = STAIR_STEPS_PER_FLIGHT - 1
    flight_run_visible = visible_steps_per_flight * STAIR_TREAD_DEPTH

    min_rel = STAIR_TREAD_DEPTH / 2.0                              # first step
    max_rel = flight_run_visible + STAIR_LANDING_LENGTH            # far end
...
def build_core_with_openings():
    """
    Core long walls now have doors on BOTH +Y and -Y sides, one per
    side per floor, centred in X, giving access to the central core slab.
    """
    ...
    for i in range(NUM_FLOORS):
        ...
        # mid (door height) band for piers
        ...
        for side_sign, suffix in ((+1, "+Y"), (-1, "-Y")):
            ...
            # vertical piers on each side of door
            for si, (a, b) in enumerate(segments):
                seg_width = b - a
                if seg_width <= 0.05:
                    continue
...
def build_elevator_shafts():
    """
    Two elevator shafts occupying the west side of the core.
    """
    x_min_e, x_max_e, y_in_min, y_in_max = elevator_bounds()
    x_center = (x_min_e + x_max_e) / 2.0
    sx       = (x_max_e - x_min_e) / 2.0

    # split the Y-range into two equal shafts (south & north)
    y_mid = 0.0
    sy_s = (0.0 - y_in_min) / 2.0
    cy_s = (y_in_min + 0.0) / 2.0

    sy_n = (y_in_max - 0.0) / 2.0
    cy_n = (0.0 + y_in_max) / 2.0
...
def build_floor_slabs():
    """
    Floor slabs:
      - left/right/north/south wings as before,
      - additional central slab inside the core between elevators and stair,
        aligned with the main slabs (this is the lobby / stair landing
        connection).
    """
    inside_x_min = -BUILDING_LENGTH / 2.0 + WALL_THICKNESS
    inside_x_max =  BUILDING_LENGTH / 2.0 - WALL_THICKNESS
    ...
    walkway_clear = 0.05
    core_slab_x_min = x_max_e + walkway_clear
    core_slab_x_max = x_stair_west - walkway_clear
...
# ------------------------------------------------------------------
# STAIRS – DOUBLE FLIGHT AT +X CORE EDGE (FROM EXPERIMENT 10)
# ------------------------------------------------------------------

def build_stairs():
    """
    Multi-storey U-shaped double-flight stair inside the core,
    placed against the +X interior wall, with:
      - discrete treads,
      - one mid-landing per storey (“in the air”),
      - connection to the central core slab at each floor.
    """
