# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 09 - CHATGPT 5.1 - V05
# HOW TO CLT
# ------------------------------------------------------------------
...
STAIR_WIDTH           = 1.2  # width across X (tread width)

# Landing inside the core (intermediate landing)
LANDING_DEPTH         = 1.2  # along Y
LANDING_THICKNESS     = SLAB_THICKNESS
...
# ------------------------------------------------------------------
# STAIRS – CORRECT DOUBLE FLIGHT WITH INTERMEDIATE LANDING
# ------------------------------------------------------------------

def build_stairs():
    """
    Build a correct double-flight stair between each pair of floors:

      - Lower flight: floor n -> intermediate landing (mid-height),
        sloping up along +Y.
      - Upper flight: intermediate landing -> floor n+1,
        sloping up along -Y.
      - Intermediate landing: separate slab inside the core.

    Both flights are placed in the eastern half of the core; the
    elevator uses the western half.
    """
    # Interior faces of core walls
    inner_x_half = CORE_LENGTH / 2.0 - WALL_THICKNESS
    inner_y_half = CORE_WIDTH  / 2.0 - WALL_THICKNESS

    # X position of stair (east side of core, clear of east wall)
    sx_flight = STAIR_WIDTH / 2.0
    sy_flight = STAIR_FLIGHT_LENGTH / 2.0
    sz_flight = STAIR_THICKNESS / 2.0

    # run/2 (horizontal projection in Y of half the flight)
    run_half = sy_flight * math.cos(STAIR_ANGLE_RAD)

    # stair centre in X: near east interior face, with clearance
    cx = inner_x_half - sx_flight - STAIR_CLEARANCE

    # intermediate landing (inside core, centred in Y)
    sy_land = LANDING_DEPTH / 2.0
    sx_land = STAIR_WIDTH / 2.0
    sz_land = LANDING_THICKNESS / 2.0
    y_mid   = 0.0

    for i in range(NUM_FLOORS):
        z0    = PODIUM_HEIGHT + i * FLOOR_HEIGHT
        z1    = z0 + FLOOR_HEIGHT
        z_mid = (z0 + z1) / 2.0

        # Intermediate landing slab (top of slab = z_mid)
        cz_land = z_mid - sz_land
        make_block(
        ...
        # ---- Lower flight: floor -> landing (angle +STAIR_ANGLE_DEG) ----
        zc1 = (z0 + z_mid) / 2.0

        # top of lower flight touches south edge of landing
        y_top1 = y_mid - sy_land
        cy1    = y_top1 - run_half  # centre so that top is at y_top1
        ...
        # ---- Upper flight: landing -> next floor (angle -STAIR_ANGLE_DEG) ----
        zc2 = (z_mid + z1) / 2.0

        # bottom of upper flight touches north edge of landing
        y_bottom2 = y_mid + sy_land
        cy2       = y_bottom2 - run_half  # for angle -, bottom is at cy+run_half
