# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 09 - CHATGPT 5.1 - V08
# HOW TO CLT
# ------------------------------------------------------------------
...
# Elevator
ELEVATOR_DEPTH         = 1.60      # depth along X, hugging -X wall
...
def elevator_bounds():
    """
    Elevators hug the -X interior wall and are shallow in X.
    """
    x_in_min, x_in_max, y_in_min, y_in_max = core_interior_bounds()
    x_min_e = x_in_min
    x_max_e = min(x_in_min + ELEVATOR_DEPTH, x_in_max - 0.5)  # keep safe margin
...
def stair_layout_params():
    ...
    # Y-position so the stair fits inside the core
...
def build_core_with_openings():
    """
    Doors on BOTH +Y and -Y sides of the core, centred in X,
    connecting to the middle core slab.
    """
...
def build_elevator_shafts():
    """
    Two elevator shafts hugging the -X interior wall.
    """
...
def build_stairs():
    ...
    for seg in range(num_segments):
        ...
        # -------- floor-level landing (at floor elevation) --------
