# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 09 - CHATGPT 5.1 - V06
# HOW TO CLT
# ------------------------------------------------------------------
...
# Stairs (discrete treads, copied from experiment_10 + adapted)
...
STAIR_LANDING_LENGTH   = 1.00      # landing depth (along Y)
STAIR_LANDING_GAP      = 0.20      # X-gap between flights
STAIR_LANDING_THICK    = 0.15
STAIR_CLEAR_CORE_Y     = 0.05      # clearance to core walls in Y
STAIR_CLEAR_ELEV_X     = 0.10      # clearance to elevator in X
...
def build_elevator_shaft():
    ...
    cx = -(inner_x_half - sx)   # pushed to -X side
    cy = 0.0
...
# ------------------------------------------------------------------
# STAIRS – DOUBLE FLIGHT INSIDE CORE (BASED ON EXPERIMENT 10)
# ------------------------------------------------------------------

def build_stairs():
    """
    Build a multi-storey U-shaped double-flight stair inside the core,
    adapted from experiment_10_chatgpt51_v03.py:

      - discrete steps (no sloped slabs),
      - constant riser height and tread depth,
      - one mid-landing per storey,
      - bottom at first residential floor,
      - clear of the elevator volume.
    """

    # ----- derived stair parameters from experiment 10 -----
    ...
    visible_steps_per_flight = steps_per_flight - 1   # omit one tread per flight
    num_levels  = NUM_FLOORS                          # floors 1..NUM_FLOORS
    num_segments = num_levels - 1                     # stairs between floors
    ...
    # ------------------------------------------------------------------
    # Place stair origin in the core, away from elevator and walls
    # ------------------------------------------------------------------
    inner_x_half = CORE_LENGTH / 2.0 - WALL_THICKNESS
    inner_y_half = CORE_WIDTH  / 2.0 - WALL_THICKNESS

    # Elevator max X
    sx_elev     = ELEVATOR_WIDTH_X / 2.0
    elev_cx     = -(inner_x_half - sx_elev)
    elev_max_x  = elev_cx + sx_elev

    # Choose ox so that the western flight just clears the elevator
    ox = (elev_max_x + STAIR_CLEAR_ELEV_X +
          stair_width + landing_gap / 2.0)

    # Y extents of stair relative to oy:
    #   min ≈ oy + tread_depth/2
    #   max ≈ oy + flight_run_visible + landing_length
    min_rel = tread_depth / 2.0
    max_rel = flight_run_visible + landing_length

    oy_min = -inner_y_half + STAIR_CLEAR_CORE_Y - min_rel
    oy_max =  inner_y_half - STAIR_CLEAR_CORE_Y - max_rel
    oy = 0.5 * (oy_min + oy_max)

    # Z origin at first residential floor plane
    oz = PODIUM_HEIGHT + FLOOR_HEIGHT
    ...
    landing_sx = (2.0 * stair_width + landing_gap) / 2.0
    ...
    for seg in range(num_segments):
        ...
        # First flight (F1) – runs +Y from floor to landing
        ...
        # Second flight (F2) – runs back in -Y from landing to next floor
        for k in range(visible_steps_per_flight):
            step_index = k + 1  # F2_01 .. F2_09
