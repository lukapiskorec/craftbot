# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 14 - Fable run, v06
...
BANDS = [(GIRT_TOPS[k], GIRT_TOPS[k + 1] - GIRT[1]) for k in range(len(GIRT_TOPS) - 1)]   # girt top .. next girt bottom
...
ST = "New/Connections"
SEAT_T, SEAT_L, SEAT_LEG = 0.012, 0.15, 0.10     # short leg: the L1 mid-row seats sit 30 mm over the shed ridge boards
...
for o in list(bpy.data.objects):
    if o.name.startswith("Beam_") and o.type == "MESH":
        ...
        seat(f"Seat_{o.name}_W", "x", x0, +1, y0, y1, zb)
        seat(f"Seat_{o.name}_E", "x", x1, -1, y0, y1, zb)

n = len([o for o in bpy.data.objects if o.type == "MESH"])
print(f"Built experiment 14 v06: {n} elements; L1 slab top {Z_L1}, terrace deck {Z_DECK_TOP:.3f}, "
