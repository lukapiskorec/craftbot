# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 14 - Fable run, v02
# Becher study: timber coaling tower refurbished as a multi-storey home.
#
# v02: knee braces lapped on the post outer faces in the girt plane
# (v01 had them on the post row, inside the new beams); ceiling joists
# beside the rafters (CMHC fig 83: joist nailed to the rafter side);
# trimmer offsets at the stair hole; the envelope: shed clapboards,
# tower infill stud walls with windows and the clerestory band, tower
# board cladding, terrace guards, head house cladding and roof boards.
#
...
for k, zt in enumerate(GIRT_TOPS):
    zb = zt - GIRT[1]
    reach = BRACE_REACH if k > 0 else 0.45        # L1: the shed roof passes underneath
    for ix in range(NX + 1):
        for iy, sgn in ((0, -1), (NY, +1)):        # long faces
            y_face = post_y(iy) + sgn * POST / 2
            if ix > 0:
                knee(f"Knee_L{k}_{ix}{iy}_W", "Existing/Tower_Braces", "x", post_x(ix), y_face, sgn, -1, zb, reach)
            if ix < NX:
                knee(f"Knee_L{k}_{ix}{iy}_E", "Existing/Tower_Braces", "x", post_x(ix), y_face, sgn, +1, zb, reach)
    for iy in range(NY + 1):
        for ix, sgn in ((0, -1), (NX, +1)):        # short faces
            x_face = post_x(ix) + sgn * POST / 2
            if iy > 0:
                knee(f"Knee_L{k}_{ix}{iy}_S", "Existing/Tower_Braces", "y", post_y(iy), x_face, sgn, -1, zb, reach)
            if iy < NY:
                knee(f"Knee_L{k}_{ix}{iy}_N", "Existing/Tower_Braces", "y", post_y(iy), x_face, sgn, +1, zb, reach)
...
framing.clad("Clap_S", SC, "x", SHED_X0 - CLAP_T, SHED_X1 + CLAP_T, SHED_Y0 - CLAP_T, SHED_Y0, CLAP_Z0, CLAP_ZTOP,
             sheet_w=CLAP_L, sheet_l=CLAP_ROW, holes=SHED_OPENINGS["S"], stagger=True)
...
def infill_openings(k, a_posts, wide):
    ...
    for i in range(len(a_posts) - 1):
        a0, a1 = a_posts[i] + POST / 2 + 0.2, a_posts[i + 1] - POST / 2 - 0.2
        if k == N_UPPER:
...
for k in range(1, N_UPPER + 1):
    ...
    for face, along, a0, a1, b0, b1, ops in (
            ("S", "x", X_IN_W, X_IN_E, W_OFF0, W_OFF0 + WALL_T, infill_openings(k, xs, True)),
            ("N", "x", X_IN_W, X_IN_E, Y_END - W_OFF0 - WALL_T, Y_END - W_OFF0, infill_openings(k, xs, False)),
            ("W", "y", W_OFF0 + WALL_T, Y_END - W_OFF0 - WALL_T, X_IN_W, X_IN_W + WALL_T, infill_openings(k, ys, k == N_UPPER)),
            ("E", "y", W_OFF0 + WALL_T, Y_END - W_OFF0 - WALL_T, X_IN_E - WALL_T, X_IN_E, infill_openings(k, ys, k == N_UPPER))):
        framing.stud_wall(f"Infill_L{k}_{face}", IW, along, a0, a1, b0, b1, z0, z1, STUD_T, SPACING,
                          grid0=GRID0, openings=ops, lintel_d=0.15)
        TOWER_HOLES[face] += ops
...
print(f"Built experiment 14 v02: {n} elements; L1 slab top {Z_L1}, terrace deck {Z_DECK_TOP:.3f}, "
