# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 14 - Fable run, v07
# Becher study: timber coaling tower refurbished as a multi-storey home.
#
# v07: cladding bands stop at the beam underside (42 mm below the girt
# underside, the beams are deeper than the girts) and the seat brackets
# are 30 mm narrower than the beam on each side, clear of the boards.
# v06 (phase 2, comparison and structural review): tower cladding as
# horizontal boards set between the posts and flush with their outer
# faces, so posts, girts and braces read on the facade as in the photo;
# brace pattern from the interior posts on the long faces and from the
# corner posts on the short faces (two per short face per level); guards
# on the three open edges of every stair void; steel seat brackets under
# every beam end at the posts (Architect's Handbook 4-10); flights start
# 5.0 m from the far wall and the voids end at the landing riser (v05 had
# two flight feet inside the void below and a 12 cm landing strip); the
# ground flight moved west with its roof hole; shed door on the east
# gable beside the window, as in the photo.
#
...
# mid post toward y = 0.
LONG_PATTERN = [(1, -1), (2, +1), (3, +1)]     # from the interior posts only; bay 1 unbraced
SHORT_PATTERN = [(0, +1), (2, -1)]             # from the corner posts, which carry no long-face lap
for k, zt in enumerate(GIRT_TOPS):
    zb = zt - GIRT[1]
    ...
    for ix, sgn in ((0, -1), (NX, +1)):            # short faces (none at L1: the shed ridge is 50 mm under that girt)
        x_face = post_x(ix) + sgn * POST / 2
        for iy, d in (SHORT_PATTERN if k > 0 else []):
...
# --- CLT slabs: one strip per bay in Y, bearing SLAB_BEAR on the beams on
# both edges, notched around the interior posts (CNC cut, 25 mm clearance),
# holed by the stair void. Convex pieces by peeling each hole's four sides.
SLAB_BEAR = 0.05
...
for k in range(1, N_UPPER + 1):
    ...
    vx0, vx1 = VOIDS[k]
    for iy in range(NY):
        ya, yb = post_y(iy) + GLB[0] / 2 - SLAB_BEAR, post_y(iy + 1) - GLB[0] / 2 + SLAB_BEAR
...
DOOR = (3.0, 4.0, None, 2.1)                 # east gable, photo: window left, door right
WIN_W = (2.0, 3.2, 0.9, 2.1)
...
framing.stud_wall("ShedWall_W", SW, "y", SHED_Y0 + WALL_T, SHED_Y1 - WALL_T, SHED_X0, SHED_X0 + WALL_T,
                  0.0, PLATE_TOP, STUD_T, SPACING, grid0=GRID0, double_top=True, openings=[WIN_W])
...
RIDGE_DROP = RIDGE_B[0] / 2 * S                    # square-edged ridge board under the board undersides
box("Ridge_Board", RR, SHED_X0, SHED_X1, RIDGE_Y - RIDGE_B[0] / 2, RIDGE_Y + RIDGE_B[0] / 2,
    RIDGE_Z - RIDGE_B[1] - RIDGE_DROP, RIDGE_Z - RIDGE_DROP)
...
deck_nogo.append((tvx0, tvx1, VOID_Y0, VOID_Y1))   # boards cantilever from the last whole joist to the void edge
...
box("HH_Ridge", HR, HH_X0 - POST / 2, X_END + POST / 2, HH_RIDGE_Y - HH_RIDGE_B[0] / 2, HH_RIDGE_Y + HH_RIDGE_B[0] / 2,
    HH_RIDGE_Z - HH_RIDGE_B[1] - HH_RIDGE_DROP, HH_RIDGE_Z - HH_RIDGE_DROP)
...
SHED_OPENINGS = {
    ...
    "W": [(2.0, 3.2, 0.9, 2.1)],
    "E": [(1.0, 2.2, 0.9, 2.1), (3.0, 4.0, 0.0, 2.1)],
}
...
for k, (z_lo, z_hi) in enumerate(TR_BAYS):
    if z_hi - z_lo < 1.0:
        continue                                                   # the stub bay under the top girt
...
# --- tower cladding: open-jointed horizontal boards 25 x 140 set between the
# posts, flush with the post outer faces, in the bands between the girts, so
# the posts, girts and knee braces stay exposed on the facade (photo: planking
# between the bunker posts). The braces lie outside the post face plane and
# the boards inside it, so the two only touch. Holed at the windows.
...
def horizontal_boards(prefix, coll, along, a0, a1, b0, b1, z0, z1, holes):
    n = 0
    z = z0
    while z < z1 - 1e-6:
        zb = min(z + CB_W, z1)
...
def vertical_boards(prefix, coll, along, a0, a1, b0, b1, z0, z1, holes):
    """Vertical open-jointed boards (kept for the head house)."""
    n = 0
    a = a0
    while a < a1 - 1e-6:
        ab = min(a + CB_W, a1)
...
BANDS = [(GIRT_TOPS[k], GIRT_TOPS[k + 1] - GLB[1]) for k in range(len(GIRT_TOPS) - 1)]   # girt top .. next beam bottom
for kb, (z0, z1) in enumerate(BANDS):
    for ix in range(NX):
        a0, a1 = post_x(ix) + POST / 2, post_x(ix + 1) - POST / 2
        horizontal_boards(f"TowerClad_S_B{kb}_{ix}", TC, "x", a0, a1, -POST / 2, -POST / 2 + CB_T, z0, z1, TOWER_HOLES["S"])
        horizontal_boards(f"TowerClad_N_B{kb}_{ix}", TC, "x", a0, a1, Y_END + POST / 2 - CB_T, Y_END + POST / 2, z0, z1, TOWER_HOLES["N"])
    for iy in range(NY):
        a0, a1 = post_y(iy) + POST / 2, post_y(iy + 1) - POST / 2
        horizontal_boards(f"TowerClad_W_B{kb}_{iy}", TC, "y", a0, a1, -POST / 2, -POST / 2 + CB_T, z0, z1, TOWER_HOLES["W"])
        horizontal_boards(f"TowerClad_E_B{kb}_{iy}", TC, "y", a0, a1, X_END + POST / 2 - CB_T, X_END + POST / 2, z0, z1, TOWER_HOLES["E"])
...
HHC_ZTOP = Z_HH_PLATE - HH_S * (WALL_T + CB_T) - 0.005    # under the rafter tails at the cladding face
...
# --- guards on the three open edges of every stair void (CMHC 1070 mm) ------
def void_guard(prefix, x0, x1, z_top, landing_side):
    """Guard on the slab / deck around the void x0..x1, y VOID_Y0..VOID_Y1:
    the two long edges plus the end opposite the landing (landing_side = +1
    when the flight lands at x1, -1 when it lands at x0)."""
    g_off = INSET + GP / 2
...
for k in range(1, N_UPPER + 1):
    vx0, vx1 = VOIDS[k]
    lands_east = (k == 1) or (k % 2 == 1)          # ground flight and the eastward flights land at x1
...
ST = "New/Connections"
SEAT_T, SEAT_L, SEAT_LEG, SEAT_IN = 0.012, 0.15, 0.10, 0.03     # short leg: the L1 mid-row seats sit 30 mm over the shed ridge boards
def seat(name, axis, a_face, dir_a, b0, b1, z_beam_bot):
    """Seat plate under the beam end (SEAT_L along the beam from the post
    face) and a vertical leg on the post face below the beam."""
    ...
    if axis == "x":
        ...
        box(name + "_Plate", ST, b0, b1, min(a_face, a_in), max(a_face, a_in), z_beam_bot - SEAT_T, z_beam_bot)
        box(name + "_Leg", ST, b0, b1, min(a_face, a_leg), max(a_face, a_leg), z_beam_bot - SEAT_T - SEAT_LEG, z_beam_bot - SEAT_T)
...
print(f"Built experiment 14 v07: {n} elements; L1 slab top {Z_L1}, terrace deck {Z_DECK_TOP:.3f}, "
