...
Consumes paths from check_routes_v04.py. Each swept disc is enclosed by a
...
for level in range(5):
    ...
    cases={label:np.load(HERE/f'route_v04_{label}{level}.npz')['path'].tolist() for label in 'WENS'}
    if level>0:
...
(HERE/'route_envelopes_v04.txt').write_text(report,encoding='utf-8')
