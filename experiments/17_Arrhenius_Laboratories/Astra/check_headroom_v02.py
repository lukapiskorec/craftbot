...
Run in Blender after opening the saved v02 model; this does not alter geometry.
"""
...
tree=ast.parse((HERE/'experiment_17_astra_v02.py').read_text(encoding='utf-8'))
...
report=f'CONTINUOUS HEADROOM: {len(walks)} convex walking regions; minimum {minimum[0]:.3f}m; {len(violations)} violations below2.10m\nGoverning: {minimum}\n'
report+='\n'.join(str(row) for row in sorted(violations))
(HERE/'headroom_v02.txt').write_text(report,encoding='utf-8')
