...
prefixes=('Column_','Corbel','Beam_','GradeBeam_','StairHeader','StairRim','Pedestal_','FootingPad_','CoreBeamLedge',
...
tops={};bottoms={}
for obj in bpy.data.objects:
    if obj.type!='MESH' or not obj.name.startswith(('CorbelSeat','CorbelWing','CorbelNib','Beam_')):continue
...
for column in columns:
    ...
    for level in range(1,6):
        ...
        for side in (-1,1):
            ...
            a,b=sorted((x+side*.3,x+side*.65));required=g2.rect(a,b,beam_y-.2,beam_y+.2)
            ...
            ok=seat_area>=.14-1e-5 and beam_area>=.14-1e-5
            seat_rows.append((column.name,level,side,seat_area,beam_area,ok))
            if not ok:seat_fail.append(seat_rows[-1])
lines.append(f'\nNOMINAL0.35x0.40m CORBEL END SEATS: {len(seat_rows)} expected geometric cases; {len(seat_fail)} exceptions. Both corbel-top and actual beam-underside union must cover0.140m2. Exceptions require local structural review; do not infer unsupported loads from missing beams alone.')
lines.extend(repr(row) for row in seat_fail)
reduced=[row for row in seat_fail if row[0]=='Column_81' and row[1]>1 and row[3]>=.14-1e-5 and row[4]>=.11375-1e-5]
alternate=[row for row in seat_fail if row[0]=='Column_81' and row[1]==1]
lines.append(f'Designer classification: {len(seat_rows)-len(seat_fail)} ordinary0.35x0.40m seats; {len(reduced)} reduced0.35x0.325m seats (18.75% width/area reduction; capacity unverified); {len(alternate)} first-raised cases use core-ledges checked below. No blanket876-case capacity/pass claim.')
...
(HERE/'supports_v06.txt').write_text('\n'.join(lines)+'\n',encoding='utf-8')
