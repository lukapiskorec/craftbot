...
model=runpy.run_path(str(HERE/'experiment_17_astra_v04.py'),run_name='__main__')
...
(HERE/'preflight_v04_pairs.txt').write_text(summary+''.join(f'{p*1000:.3f} mm  {a}  x  {b}\n' for p,a,b in hits))
(HERE/'preflight_v04_summary.txt').write_text(summary+format_families(families(hits),70)+'\nFLOATING\n'+'\n'.join(floating))
...
runpy.run_path(str(HERE/'check_headroom_v04.py'),run_name='__main__')
