...
for name,ff in faces.items():
    ...
    for axis,sign,z,poly,bb in ff:
        ...
        if y1-y0<5 or x1-x0<.5 or abs(area(poly)-(x1-x0)*(y1-y0))>1e-5 or any(min(abs(y-d) for d in datums)>1e-4 for y in (y0,y1)):
            excluded+=1;continue
        supports=[(n,f) for n,f in index.get((2,1,round(z,4)),[]) if n.startswith(('PerimeterLedger','Beam_','EdgeBeam','StairHeader','StairRim','CoreWall','CoreRoofLedge','Corbel','Column_')) and n!=name and abs(f[2]-z)<1e-5 and overlap(bb,f[4])]
        for side in (-1,1):
...
if '--priority' not in sys.argv:
    (HERE/'slab_end_faces_v05.json').write_text(json.dumps([dict(name=r[0],side=r[1],bounds=bounds[r[0]],required_area=r[2],actual_area=r[3],conservative_full_width_depth=r[4],passed=r[5],supports=r[6]) for r in rows],indent=2),encoding='utf-8')
...
lines.append('The bottom-face audit includes every actual mesh object as a possible support; rail/edge-only contact yields no horizontal support area. Soil below foundation bottom is not modelled and is not itself a foundation discontinuity. Raised terrace surface fill/under-support must be distinguished from a deliberately diagrammatic Site proxy.')
...
output=HERE/('structural_priority_v05.txt' if '--priority' in sys.argv else 'structural_followup_v05.txt')
output.write_text('\n'.join(lines)+'\n',encoding='utf-8')
