...
Consumes paths from check_routes_v07.py. Each swept disc is enclosed by a
...
SNAPSHOT=runpy.run_path(str(HERE/'snapshot_v07.py'))['identity']()
...
rows=[];failures=[];tangent_faces=[];limiting_routes=[]
for level in range(5):
    z=3.6*level;floors=[];obstacles=[];ceilings=[]
    for obj,points,zlo,zhi in meshes:
        if zhi<z-.001 or zlo>z+2.10:continue
        # A flat face tangent to the2.10m envelope has no obstructing volume.
        # Existing Blender coordinate seam allowance is20micrometres; log
        # each near-tangent clearance instead of silently widening a route.
        if zlo>=z+2.10-.00002:
            tangent_faces.append((level,obj.name,zlo-z))
            poly=hull([(p.x,p.y) for p in points if abs(p.z-zlo)<2e-5])
            if len(poly)>2:ceilings.append((obj.name,poly,bounds(poly),zlo-z))
            continue
    ...
    cases={label:np.load(HERE/f'route_v06_{label}{level}.npz')['path'].tolist() for label in 'WENS'}
    if level>0:
    ...
    for label,path in cases.items():
        ...
        simplified.append(path[-1]);uncovered=0.;wide_uncovered=0.;hits=set();max_seam=0.;limiting={}
        ...
        for p,q in zip(simplified,simplified[1:]):
            ...
            for name,ceiling,cb,clearance in ceilings:
                if overlaps(bb,cb) and g2.area(intersection(tube,ceiling))>1e-7:limiting[name]=clearance
        ...
        if limiting:limiting_routes.append((label,level,sorted(limiting.items())))
        if not ok:failures.append((label,level,uncovered,sorted(hits)))
report=SNAPSHOT+'\nCONTINUOUS1.20m ROUTE/2.10m HEIGHT ENVELOPES (insert1.40m);32-sided circumscribed discs; actual-mesh floor union and clipped solids\n'+'\n'.join(rows)+'\nFailures: '+repr(failures)+'\nNear-tangent upper faces, actual clearance: '+repr(tangent_faces)+'\nTolerance:20micrometre coordinate allowance at exactly-tangent2.10m upper boundary and maximum inscribed diameter for float32 floor/doorway seams,1e-7m2 per obstacle intersection; no engineering/code claim.\n'
(HERE/'route_envelopes_v07.txt').write_text(report,encoding='utf-8')
limits=[v for label,level,objects in limiting_routes for name,v in objects]
(HERE/'route_ceiling_minima_v07.txt').write_text(SNAPSHOT+'\nActual near-tangent ceiling patches intersecting swept route envelopes: '+repr(limiting_routes)+'\nRoute/object pairs: '+str(len(limits))+'; affected routes: '+str(len(limiting_routes))+'; actual minimum metres: '+str(min(limits,default=None))+'\nThese gallery ceilings are distinct from the368 stair-region minimum2.700m.\n',encoding='utf-8')
