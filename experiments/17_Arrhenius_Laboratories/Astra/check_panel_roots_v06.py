...
identity=runpy.run_path(str(HERE/('snapshot_v05.py' if baseline else 'snapshot_v06.py')))['identity']()
...
secondary=('Panel_','Window_','SouthFineStrip_','SouthFascia','SouthVent','SouthBracket','SouthRibbon','SouthSide','SouthIncline')
...
(HERE/('panel_baseline_v05_for_v06.json' if baseline else 'panel_roots_v06.json')).write_text(json.dumps(report,indent=2),encoding='utf-8')
detail=[]
...
if not baseline:(HERE/'panel_root_gaps_v06.txt').write_text(identity+'\n'+repr(detail)+'\n',encoding='utf-8')
