...
audit = runpy.run_path(str(HERE / 'structural_followup_v05.py'))
...
lines = [runpy.run_path(str(HERE / 'snapshot_v05.py'))['identity']()]
failures = []
...
for label, rect, z, minimum, prefixes in (
    ...
    ('Landing top seat', (33.15,35.45,95.80,96.00), 2.50, .46, ('CourtLandingLedge',))):
    poly = g2.rect(*rect)
    seated, supports = support_union(poly, z, prefixes)
    assert_case(label, minimum-seated <= seam_allowance(poly),
                f'required {minimum:.9f}m2; seated {seated:.9f}m2; {supports}')
...
for name in sorted(n for n in faces if n.startswith('NorthTerrace')):
    for axis, sign, z, poly, _ in faces[name]:
        ...
        assert_case(name+' supported underside', area(supported)-seated <= seam_allowance(supported),
                    f'required {area(supported):.9f}m2; seated {seated:.9f}m2; '
                    f'explicit north cantilever {area(strip):.9f}m2; {supports}')
assert_case('Terrace total', terrace_required-terrace_seated < .001,
            f'support-required {terrace_required:.9f}m2; seated {terrace_seated:.9f}m2; '
            f'30mm north cantilever {cantilever:.9f}m2; nominal cantilever .507m2')

old_ends = json.loads((HERE/'slab_end_faces_v04.json').read_text())
new_ends = json.loads((HERE/'slab_end_faces_v05.json').read_text())
...
for row in former_fails:
    ...
    seated, supports = support_union(poly,z,('PerimeterLedger','Beam_','EdgeBeam','Column_'))
...
for name in sorted(n for n in faces if n.startswith(('PerimeterLedger','CourtLandingLedge'))):
    prefixes = ('Beam_', 'Column_', 'EdgeBeam') if name.startswith('PerimeterLedger') else ('RetainingWall',)
...
for start in sorted(ledger_graph):
    ...
    while queue:
        name=queue.popleft()
        if name.startswith(('Beam_','Column_','EdgeBeam','RetainingWall')):
            target=name;break
...
for name in sorted(n for n in faces if n.startswith(('TerraceWall','TerraceFooting','TerraceFill','TerraceBase',
                                                     'CourtStairFooting','RetainingBase'))):
    for axis,sign,z,poly,_ in faces[name]:
        if axis!=2 or sign!=-1:
            continue
        seated,supports=support_union(poly,z,('Terrace','CourtSubsoil','Insert','Retaining','Footing','GradeBeam','Pedestal'))
        assert_case(name+' ground/support union',area(poly)-seated<=seam_allowance(poly),
                    f'required {area(poly):.9f}m2; seated {seated:.9f}m2; {supports}')
...
(HERE/'completion_v05.txt').write_text('\n'.join(lines)+'\n',encoding='utf-8')
