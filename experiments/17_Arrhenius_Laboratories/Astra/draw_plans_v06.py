...
for name,title,(xa,xb,ya,yb),level,labels in SPECS:
    ...
    svg=[f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}"><rect width="100%" height="100%" fill="#f4f2ed"/>',f'<text x="45" y="40" font-family="Arial" font-size="26">{html.escape(title)}</text>',f'<text x="45" y="70" font-family="Arial" font-size="15">Frozen v06 mesh | solid wall sections at floor +1.20m | inferred scale | north up</text>','<defs><clipPath id="crop"><rect x="'+str(ox)+'" y="'+str(oy)+'" width="'+str(scale*(xb-xa))+'" height="'+str(scale*(yb-ya))+'"/></clipPath></defs><g clip-path="url(#crop)">']
    ...
    content=''.join(svg);(HERE/f'plan_v06_{name}.svg').write_text(content,encoding='utf-8')
    (HERE/f'plan_v06_{name}.html').write_text('<!doctype html><html><body style="margin:0">'+content+'</body></html>',encoding='utf-8')
