...
strips=[('Paving',21.9,23.1,-.9,.3,0.),('Porch',21.9,23.1,.3,1.5,0.),
        ('CourtPath',33.75,34.95,65.8,89.7,0.)]
...
rows=[runpy.run_path(str(HERE/'snapshot_v07.py'))['identity']()];failures=[]
...
rows+=['Removed Lawn/CourtGarden are excluded by concept9; their surfaces are unmodeled. Historical grade transitions were150/100mm; nineteen tread tops plus landing make20 x0.150m rises to3.000m. Consecutive strip boundaries coincide; these are stepped pedestrian routes, not accessible ramps.',f'Failures: {failures}']
...
for name,b in byname.items():
    if not name.startswith(('GradeBeam_',)) or abs(b[5]-paving[4])>2e-5:continue
...
unmodeled=sum(g2.area(p) for p in remaining)
bearing=abs(sum(areas.values())-1.20)<1e-5 and abs(unmodeled-2.40)<1e-5
rows.append(f'Approach retained concrete seat and explicit unmodeled earth-bearing remainder atz{paving[4]:.3f}: actual face areas{areas}; PASS{bearing}; unmodeled base/ground footprint{unmodeled:.6f}m2; no mesh-proved soil claim.')
...
(HERE/'public_routes_v07.txt').write_text('\n'.join(rows)+'\n',encoding='utf-8')
