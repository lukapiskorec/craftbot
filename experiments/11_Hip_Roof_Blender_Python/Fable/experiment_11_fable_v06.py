# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 (Fable run) - V06
...
#
# v05 (review against the CMHC figures + structural review):
#   * doubled ceiling joist at the hip ends next to the stub joists (Fig. 85)
#   * 38 x 89 struts >= 45 deg from the centre bearing wall to the main
#     rafters (p. 126, Fig. 86) - rafter slope span 5.25 m > 4.7 m example
#   * strongback (2 x 38 x 89 on edge) + struts under the E / W hip-end
#     jack rafters (p. 126)
#   * wing centre bearing wall, wing ceiling joists lapped over it
#     (7.2 m single span was too long for 38 x 184 joists)
# v06 (second pass through the PDF figures):
#   * dormer valley rafters 38 x 235 (>= 50 mm deeper than the 140 jacks)
#   * Fig. 90 rake detail: 19 x 89 nailing strip on the rafter over the
#     gable wall, rake blocking end-nailed to the fly rafter
#   * strongback + three struts under the wing-S hip-end jacks, bearing on
#     the wing partition (mirrors the E / W hip ends)
# ------------------------------------------------------------------
...
D_RAF_D, D_RIDGE_D, D_VAL_D = 0.140, 0.184, 0.235   # valley = jack + 95 (>= 50, p. 126)
...
WING_MID = 0.5 * (WING_Y + -MAIN_Y)                  # -7.8: wing bearing partition
wall("Wall_WingCentre", WALLS, -WING_X + STUD_D, WING_X - STUD_D, WING_MID - STUD_D / 2, WING_MID + STUD_D / 2)
...
for k, x in enumerate(grid_main):
    ...
    if abs(x) < WING_X - STUD_D - T:
        joist(f"Joist_WingS_{k:02d}", (x - T, WING_Y), (0, 1), WING_MID - WING_Y + LAP, [WING_S.below()])
        joist(f"Joist_WingN_{k:02d}", (x + T, WING_MID - LAP), (0, 1), -MAIN_Y - WING_MID + LAP, [])

# doubled joists at the hip ends, stub joists beside the jack rafters (Fig. 85)
X_DBL = grid_main[-1] + 2 * T
joist("Joist_N_Double_E", (X_DBL, -LAP), (0, 1), MAIN_Y + LAP, [MAIN_N.below()])
joist("Joist_N_Double_W", (-X_DBL, -LAP), (0, 1), MAIN_Y + LAP, [MAIN_N.below()])
joist("Joist_S_Double_E", (X_DBL - 4 * T, -MAIN_Y), (0, 1), MAIN_Y + LAP, [MAIN_S.below()])
joist("Joist_S_Double_W", (-X_DBL + 4 * T, -MAIN_Y), (0, 1), MAIN_Y + LAP, [MAIN_S.below()])
X_LAST = X_DBL + T / 2                      # outer face of the doubled joist
...
# intermediate support (p. 126): 38 x 89 struts from the centre bearing
# wall to the main rafters, >= 45 deg, standing on the lapped joists
STRUT_Y = 1.5
Z_STRUT0 = Z_PL + JOIST_D


def strut(name, x, sign, roof):
    z1 = roof.z(x, sign * STRUT_Y) - RAF_D / COS
    m = (z1 - Z_STRUT0) / STRUT_Y
    member(name, RF_MAIN, (x, 0.0), (0, sign), m, Z_STRUT0 + 0.06, TIE_D, T, 0.0, STRUT_Y + 0.4,
           [vz(Z_STRUT0, 1), roof.below(-RAF_D)])


for k, x in enumerate(grid_main):
    if abs(x) <= RIDGE_END + 1e-6 and round(x, 4) not in FAC["N"].skip:
        strut(f"Strut_N_{k:02d}", x + beside("N", x), 1, MAIN_N)
    if 1.8 - 1e-6 <= abs(x) <= RIDGE_END + 1e-6:
        strut(f"Strut_S_{k:02d}", x - T, -1, MAIN_S)

# strongback under the hip-end jacks (2 x 38 x 89 on edge) with struts
# radiating from the centre bearing wall
for sign, tag in ((1, "E"), (-1, "W")):
    x_sb = sign * 5.1
    roof = MAIN_E if sign > 0 else MAIN_W
    y_ext = MAIN_X - abs(x_sb) - 0.1                      # stays clear of the hips
    for i in range(2):
        x0, x1 = x_sb - T + i * T, x_sb + i * T
        z_sb = min(roof.z(x0, 0.0), roof.z(x1, 0.0)) - RAF_D / COS - 0.001   # under the lower edge
        box(f"Strongback_{tag}_{i}", RF_MAIN, x0, x1, -y_ext, y_ext, z_sb - TIE_D, z_sb)
    z_sb = min(roof.z(x_sb - T, 0.0), roof.z(x_sb + T, 0.0)) - RAF_D / COS - 0.001
    for sg in (1, -1):
        m = (z_sb - TIE_D - Z_PL) / 1.0
        member(f"StrongbackStrut_{tag}_{'p' if sg > 0 else 'm'}", RF_MAIN, (x_sb, sg * 0.04), (0, sg), m,
               Z_PL + 0.06, TIE_D, T, 0.0, 1.3, [PLATE_TOP, vz(z_sb - TIE_D, -1)])

# wing-S hip end: strongback at y = -9.0 under the jacks, three struts
# (x = 0, +-1.2, between the lapped joists) from the wing partition
Y_SB = -9.0
for i in range(2):
    y0, y1 = Y_SB - T + i * T, Y_SB + i * T
    z_sb = min(WING_S.z(0.0, y0), WING_S.z(0.0, y1)) - RAF_D / COS - 0.001
    x_ext = -(Y_SB - WING_RIDGE_S) - 0.1   # facet half-width at the strongback (hip x = -(y + 7.2)), clear of the hips
    box(f"Strongback_wS_{i}", RF_WING, -x_ext, x_ext, y0, y1, z_sb - TIE_D, z_sb)
z_sb = min(WING_S.z(0.0, Y_SB - T), WING_S.z(0.0, Y_SB + T)) - RAF_D / COS - 0.001
for j, xs in enumerate((-1.2, 0.0, 1.2)):
    run = (WING_MID - STUD_D / 2) - Y_SB
    m = (z_sb - TIE_D - Z_PL) / run
    member(f"StrongbackStrut_wS_{j}", RF_WING, (xs, WING_MID - STUD_D / 2), (0, -1), m,
           Z_PL + 0.06, TIE_D, T, 0.0, run + 0.3, [PLATE_TOP, vz(z_sb - TIE_D, -1)])
...
def dormer(tag, fac, ac):
    ...
    # Fig. 90: 19 x 89 nailing strip on the outer face of the rafter over the
    # gable wall; blocking toenailed to the strip, end-nailed to the fly rafter
    s_strip = D_FRONT + D_WALL / 2 - T / 2 - BOARD_T          # strip face positions
    for sg in (1, -1):
        member(f"{tag}_NailStrip_{'p' if sg > 0 else 'm'}", coll, P(sg * (D_OUT + D_OV_SIDE), s_strip + BOARD_T / 2),
               -sg * A, S, z_dT0 - S * D_OV_SIDE - 0.01, D_WALL, BOARD_T, 0.0, D_OUT + D_OV_SIDE - T / 2,
               [hs(P(sg * T / 2, 0), sg * A), vz(z_dpt, 1)])
    for j, a in enumerate((-0.8, -0.4, 0.4, 0.8)):
        member(f"{tag}_RakeBlock_{j}", coll, P(a, s_rake + T), Sd, 0.0, z_dT0 + S * (D_OUT - abs(a) - T / 2) - 0.001,
               D_RAF_D, T, 0.0, s_strip - (s_rake + T))
...
print(f"[exp11 v06] Z_T0={Z_T0:.3f} Z_R={Z_R:.3f} HAP={HAP:.3f} dormer apex s={DORMER_GEOM[0]['s_apex']:.3f}")
print(f"[exp11 v06] objects: {len([o for o in bpy.data.objects if o.type == 'MESH'])}")
...
print(f"[exp11 v06] boards: {total_boards}")
...
def dormer_facade(dg):
    ...
    z_clad_top = z_dT0 + S * D_OUT + 0.1          # up to the gable apex
    z_base = roof.z(*P(0, D_FRONT)[:2]) + BOARD_T / COS
    under_rafters = [DR[1].below(-D_RAF_D), DR[-1].below(-D_RAF_D),
                     vz(dg["z_dR"] - S * T / 2 - 0.001 - D_RIDGE_D, -1)]     # under the dormer ridge
