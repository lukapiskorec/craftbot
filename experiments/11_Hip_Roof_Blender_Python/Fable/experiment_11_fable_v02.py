# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 (Fable run) - V02
...
def dormer(tag, fac, ac):
    ...
    for sg in (1, -1):
        ...
        member(f"{tag}_Valley_{'p' if sg > 0 else 'm'}", coll, foot, d, HIP_M,
               roof.z(foot.x, foot.y) - 0.001, D_VAL_D, T, -0.1, 3.0,
...
print(f"[exp11 v02] Z_T0={Z_T0:.3f} Z_R={Z_R:.3f} HAP={HAP:.3f} dormer apex s={DORMER_GEOM[0]['s_apex']:.3f}")
print(f"[exp11 v02] objects: {len([o for o in bpy.data.objects if o.type == 'MESH'])}")
