# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 - CHATGPT 5.1 - V16
# HIP ROOF
# ------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DORMER ON NORTH SLOPE – REUSABLE BUILDER
# ---------------------------------------------------------------------------
...
def build_dormer_on_north_slope(
    ...
    # ---------------------------------------------------------------
    # 1. Side top plates only (front window framing removed)
    # ---------------------------------------------------------------
    ...
    # ---------------------------------------------------------------
    # 3. Dormer common rafters (denser, even spacing)
    # ---------------------------------------------------------------
    ...
    # ---------------------------------------------------------------
    # 5. Upper double header between supporting rafters
    # ---------------------------------------------------------------
    ...
    if num_dormer_jacks > 0:
        for j in range(num_dormer_jacks):
            ...
            # Intersect x_r = valley x for left and right
            candidates = []

            # Left valley
            if abs(valley_top_L.x - bottom_L.x) > 1e-6:
                tL = (header_center_x - bottom_L.x) / (valley_top_L.x - bottom_L.x)
                if 0.0 <= tL <= 1.0:
                    P_L = bottom_L.lerp(valley_top_L, tL)
                    candidates.append(P_L)

            # Right valley
            if abs(valley_top_R.x - bottom_R.x) > 1e-6:
                tR = (header_center_x - bottom_R.x) / (valley_top_R.x - bottom_R.x)
                if 0.0 <= tR <= 1.0:
                    P_R = bottom_R.lerp(valley_top_R, tR)
                    candidates.append(P_R)

            # For jack rafters we interpolate in Y along the valley lines
            ...
            # top brace tying the two valley jacks together
            brace_center_x = 0.5 * (P_valley_L.x + P_valley_R.x)
            # Shift in -Y by one profile thickness (stud_depth)
            brace_y = y_j - stud_depth
            brace_z = P_valley_L.z + 0.6 * (ridge_z - P_valley_L.z)
            # Shortened to 0.48 of valley span for better visual match
    ...
    for idx, x_r in enumerate(interior_x):
        # Try projecting onto both left and right valley rafters
        candidates = []

        # Left valley candidate
        if abs(valley_top_L.x - bottom_L.x) > 1e-6:
...
def build_scene():
    ...
    # Two rafter bays between adjacent dormers – centers at ±5 × spacing.
    center_offsets = [
        -5.0 * rafter_spacing,
        0.0,
        5.0 * rafter_spacing,
    ...
    support_half = 1.5 * rafter_spacing  # where side studs land on rafters
    ...
    for d_id, cx in enumerate(center_offsets):
        ...
        build_dormer_on_north_slope(
            ...
            rafter_spacing=0.40,  # denser dormer rafters
            dormer_id=d_id,
        )

    # Add one full rafter between each pair of dormers on the north side
    gap_xs = []
