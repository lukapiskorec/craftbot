# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 - CHATGPT 5.1 - V07
# HIP ROOF
# ------------------------------------------------------------------
...
# -----------------------------------------------------------
# DORMER – RIDGE & PLATES ROTATED 90° AROUND Z
# -----------------------------------------------------------
...
def build_dormer_rotated(main_geom,
                         ...
                         wall_height=0.7):
    """
    Gable dormer on the north slope whose ridge and main side plates
    are rotated 90 degrees around Z compared with the previous version:

    - Dormer_Ridge now runs along Y.
    - Dormer_Plate_Front and Dormer_Plate_Back now run along Y (at
      x = -width/2 and x = +width/2).
    - Dormer rafters run from these side plates into the ridge.
    - Valleys and valley jacks are adjusted to this new orientation.
    """
    ...
    # --------- Opening location on roof (same logic) ---------
    center_x = 0.0
    half_dX = dormer_width_x * 0.5
    x_L = center_x - half_dX
    x_R = center_x + half_dX

    # Put dormer somewhere mid-slope and give it limited length in Y
    ...
    # Opening corners used for header / reference
    ...
    # --------- Double header across cut rafters (unchanged orientation) ---------
    create_prismatic_member(
    ...
    # --------- Rotated wall top plates (side walls, running along Y) ---------
    # Make wall plate height roughly above middle of roof plane
    base_mid_z = 0.5 * (z_front_plane + z_back_plane)
    # clamp to keep dormer below main ridge
    max_wall = ridge_height * 0.9 - base_mid_z
    wall_h = min(wall_height, max(0.3, max_wall))

    top_plate_z = base_mid_z + wall_h

    # These are the two side plates after a 90° rotation around Z.
    create_prismatic_member(
        "Dormer_Plate_Front",  # now at x = x_L, along Y
    ...
    create_prismatic_member(
        "Dormer_Plate_Back",   # now at x = x_R, along Y
    ...
    # Optional gable plates at front/back (along X)
    create_prismatic_member(
    ...
    # --------- Rotated dormer ridge (along Y) ---------
    # Ridge height based on run in X
    local_run = half_dX
    rise_local = local_run * k
    dormer_ridge_z = top_plate_z + rise_local
    ...
    # --------- Dormer rafters (from both side plates into ridge) ---------
    ...
    # --------- Valley rafters (adjusted to new dormer orientation) ---------
    # Start at front top corners of side walls, end at eave line
    offset_y = half_W - y_front

    valley_top_L = Vector((x_L, y_front, top_plate_z))
    valley_top_R = Vector((x_R, y_front, top_plate_z))

    valley_bottom_L = Vector((x_L - offset_y * 0.8, half_W, plate_z))
    valley_bottom_R = Vector((x_R + offset_y * 0.8, half_W, plate_z))
    ...
    # --------- Valley jack rafters from rotated ridge to valleys ---------
    n_vj = 2
    for j in range(1, n_vj + 1):
        ...
        vL = valley_top_L.lerp(valley_bottom_L, s)
        vR = valley_top_R.lerp(valley_bottom_R, s)
...
def build_scene():
    ...
    # North dormer with ridge and plates rotated 90° around Z
