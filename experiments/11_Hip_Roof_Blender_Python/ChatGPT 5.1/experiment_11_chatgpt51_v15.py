# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 - CHATGPT 5.1 - V15
# HIP ROOF
# ------------------------------------------------------------------
...
def build_dormer_on_north_slope(
    ...
    if num_dormer_jacks > 0:
        for j in range(num_dormer_jacks):
            ...
            # Shorten to 0.6x of previous 0.8 factor => 0.48 overall
    ...
    for idx, x_r in enumerate(interior_x):
        ...
        # top segment – these are the Dormer_TrimmedTop members
        create_prismatic_member(
...
def build_scene():
    ...
    # We want two full rafter bays between adjacent dormers.
    # Each dormer side stud is at ±1.5 * spacing from its center,
    # so to get a 2*spacing gap: D - 3*spacing = 2*spacing => D = 5*spacing.
    center_offsets = [
        -5.0 * rafter_spacing,
        0.0,
        5.0 * rafter_spacing,
