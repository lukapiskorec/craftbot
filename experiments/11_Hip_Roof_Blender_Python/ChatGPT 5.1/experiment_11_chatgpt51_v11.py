# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 - CHATGPT 5.1 - V11
# HIP ROOF
# ------------------------------------------------------------------
...
def build_dormer_on_north_slope(
    ...
    # ---------------------------------------------------------------
    # 11. Lower double header below dormer (on main roof plane)
    # ---------------------------------------------------------------
    lower_header_y = 0.5 * (front_y + y_eave)
    lower_header_z = z_on_main_roof(lower_header_y)

    box(
        name="Dormer_LowerHeader_A",
        center=(header_center_x, lower_header_y - header_offset, lower_header_z),
        size=(header_len_x, stud_depth, main_rafter_depth),
    )
    box(
        name="Dormer_LowerHeader_B",
        center=(header_center_x, lower_header_y + header_offset, lower_header_z),
        size=(header_len_x, stud_depth, main_rafter_depth),
    )

    # ---------------------------------------------------------------
    # 12. Trimmed main rafters between supporting rafters
    #     - top segments: main ridge -> valley
    #     - bottom segments: Plate_Long_N -> lower double header
    # ---------------------------------------------------------------
    for idx, x_r in enumerate(interior_x):
        ...
        # bottom segment
        H = Vector((x_r, lower_header_y, z_on_main_roof(lower_header_y)))
        create_prismatic_member(
    ...
    # ---------------------------------------------------------------
    # 13. Top braces tying dormer rafters
    # ---------------------------------------------------------------
