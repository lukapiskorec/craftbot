# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 - CHATGPT 5.1 - V06
# HIP ROOF
# ------------------------------------------------------------------
...
# -----------------------------------------------------------
# DORMER – MORE REALISTIC GEOMETRY
# -----------------------------------------------------------

def build_dormer_realistic(main_geom,
                           dormer_width=3.0,
                           dormer_depth=1.2,
                           wall_height=0.7):
    """
    Compact gable dormer on the north slope of the main roof.

    Key ideas (to resemble a typical dormer in elevation):
    - Ridge of dormer parallel to main ridge.
    - Dormer located lower on the north slope, with shallow depth.
    - Top of dormer walls kept below main ridge so the dormer
      reads as a secondary volume.
    - Shorter valley rafters and a few valley jack rafters.
    """
    ...
    # --------- Dormer opening in plan (shallower and lower) ---------
    center_x = 0.0
    half_dW = dormer_width * 0.5
    x_L = center_x - half_dW
    x_R = center_x + half_dW

    # Position: 1.0 m down from ridge in plan, then shallow depth upslope
    y_front = half_W - 1.0          # towards eave
    y_back  = y_front - dormer_depth
    y_mid   = 0.5 * (y_front + y_back)

    z_front_plane = roof_z_at(0.0, y_front)
    z_back_plane  = roof_z_at(0.0, y_back)

    # Clamp wall height so back plate stays below main ridge
    max_wall_at_back = ridge_height * 0.95 - z_back_plane
    wall_h = min(wall_height, max(0.3, max_wall_at_back))
    ...
    # --------- Double rafters on main roof (sides of opening) ---------
    ...
    # --------- Double header across bottom of opening ---------
    create_prismatic_member(
    ...
    top_plate_z_front = z_front_plane + wall_h
    top_plate_z_back  = z_back_plane  + wall_h
    ...
    # --------- Dormer gable roof (ridge parallel to main ridge) ---------
    local_run = 0.5 * dormer_width
    rise_local = local_run * k
    dormer_ridge_z = max(top_plate_z_front, top_plate_z_back) + rise_local

    # Ensure dormer ridge is visibly below main ridge
    ...
    # --------- Valley rafters (shorter, more realistic) ---------
    ...
    for sign, tag in ((-1, "L"), (1, "R")):
        ...
        # Slightly shorten so valley meets roof just above eave line
        valley_bottom = Vector((x_corner + sign * offset_y * 0.8,
                                half_W,
                                plate_z))
        ...
        for j in range(1, n_vj + 1):
            ...
            # Ridge point biased towards the corresponding side
            if sign < 0:
...
def build_scene():
    ...
    # North dormer – compact, lower, with shorter valleys
    build_dormer_realistic(
        main_geom=main_geom,
        dormer_width=3.0,
        dormer_depth=1.2,
        wall_height=0.7,
