# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 11 - CHATGPT 5.1 - V10
# HIP ROOF
# ------------------------------------------------------------------
...
def build_main_hip_roof(length,
    ...
    """
    Main rectangular hip roof with plates, common rafters, hips, king rafters,
    jack rafters, and ceiling joists spanning Y.

    north_clear_range = (x_min, x_max): skip common rafters on north side
    within this X range (dormer opening).

    south_clear_half_x: skip common rafters on south side whose |x| is
    below this value (T-wing intersection zone).
    """
    ...
    if ridge_len > 0.0:
        ...
        for i in range(n_steps + 1):
            ...
            # north side: skip rafters in dormer opening zone
            skip_N = False
            ...
            # south side: skip rafters in T-wing intersection zone
            skip_S = False
...
# -----------------------------------------------------------
# SOUTH T-WING WITH VALLEYS AND WING JACK RAFTERS
# -----------------------------------------------------------
...
def build_T_wing_rotated(main_geom,
    ...
    # Original wing ridge (between south eave and main building)
    ...
    # Extension of wing ridge up to main ridge board
    create_prismatic_member(
        "Wing_Ridge_Ext",
        start=(0.0, y_north, ridge_height_wing),
        end=(0.0, 0.0, ridge_height_main),
        width=raf_w,
        depth=raf_d,
    )
    ...
    # Valley intersection between wing ridge and main south slope
    ...
    # Jack rafters from MAIN ridge to valleys (existing)
    n_jacks = 4
    ...
    # NEW: wing-side jack rafters between Wing_Ridge and valleys
    n_wing_jacks = 4
...
# -----------------------------------------------------------
# BOX & RAFTER HELPERS (from experiment_12 v10)
# -----------------------------------------------------------
...
# -----------------------------------------------------------
# DORMER ON NORTH SLOPE – ADAPTED FROM experiment_12 v10
# -----------------------------------------------------------
...
def build_dormer_on_north_slope(
        ...
        num_dormer_jacks=3):
    """
    Build a doghouse dormer identical in geometry to experiment_12 v10,
    but resting on the NORTH slope of the existing hip roof.
    """
    ...
    # 5. X-grid for double header (alignment with host rafters)
    ...
    # 8. Jack rafters between valleys and dormer ridge
    ...
    # 10. Vertical side studs (roof -> dormer rafters)
...
def build_scene():
    # Main roof parameters (scaled up ~1.5x)
    length = 18.0   # was 12.0
    width = 9.0     # was 6.0
    plate_z = 0.0
    ...
    # Dormer parameters (same as before, but we use them also
    # to define the north-clear zone for rafters)
    dormer_width = 2.0
    dormer_depth = 1.6
    ...
    dormer_half = dormer_width * 0.5
    x_left_cut = -dormer_half + dormer_stud_width * 0.5
    x_right_cut = dormer_half - dormer_stud_width * 0.5
    north_clear_range = (x_left_cut - 0.05, x_right_cut + 0.05)
    ...
    # Build main hip roof, skipping rafters under dormer (north)
    # and under T-wing (south).
    ...
    # South T-wing with extended ridge and wing jack rafters
    ...
    # North dormer, as in experiment_12 v10
