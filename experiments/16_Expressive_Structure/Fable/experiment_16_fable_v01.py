# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 16, Fable run, v01
#
...
SPLICE = {"A": (4.55, 9.10), "B": (2.60, 7.15)}      # stud splices on rail centres k = 7, 14 / 4, 11 (3.2: staggered, on a support)
...
def ring_nodes(z_e, i):
    """inner nodes [(s, (x, z), run)], outer vertices [(s, (x, z))] incl. both feet."""
    runs, S = ring_outline(z_e)
    inner, outer = [], [(0.0, run_pt(runs[0], 0.0, ring_depth(0.0, i)))]
...
def stud_slat(prefix, coll, kind, sgn, a0, a1, z0, top, splices):
    ...
    cuts = enforce_max(cuts, RAIL_CENTRES)
    for m in range(len(cuts) - 1):
...
def stud_pair(prefix, coll, kind, sgn, a_c, z0, top):
    for ab, (a0, a1) in (("A", (a_c - F_T, a_c)), ("B", (a_c, a_c + F_T))):
        stud_slat(f"{prefix}{ab}", coll, kind, sgn, a0, a1, z0, top, SPLICE[ab])
...
def build_ring(i):
    y = RING_Y0 + MOD * i
    runs, inner, outer = ring_nodes(Z_E, i)
    ...
    # outer chord: one sandwich pair per segment, mitred on the bisector at every shared vertex
    ...
    def inner_clips(idx, k_own):
        pn = inner[idx][1]
        n_own = runs[k_own]["n"]
        out = [hs(P3(pn) - N3(n_own) * F_D / 2, N3(n_own))]
        others = {inner[idx][2]} | ({inner[idx - 1][2]} if idx > 0 else set())
        for k in others - {k_own}:               # corner node: stay inside the other run's outer face
            n_o = runs[k]["n"]
            out.append(hs(P3(pn) + N3(n_o) * F_D / 2, -N3(n_o)))
        return out
    ...
    for j in range(len(inner) - 1):
        ...
        slat_bar(f"RingWeb_{i:02d}_{j:02d}R", coll_w, F, n0, Q[m], (0, 0, 1), w_off=-F_T / 2, out=(0, 1, 0), ext=(0.2, 0.2),
                 clips=[vz(0.0, +1)] + inner_clips(j, k_own) + outer_clips(m))
        slat_bar(f"RingWeb_{i:02d}_{j:02d}F", coll_w, F, Q[m], n1, (0, 0, 1), w_off=F_T / 2, out=(0, 1, 0), ext=(0.2, 0.2),
                 clips=[vz(0.0, +1)] + inner_clips(j + 1, k_own) + outer_clips(m))
    # R-10 samples: three outer vertices per truss sit at d(s, i) from the inner outline
    for m in (5, len(outer) // 2, len(outer) - 6):
        s, (qx, qz) = outer[m]
...
ring_objs = list(bpy.data.collections["Ring"].all_objects)
rx0, rx1, ry0, ry1, rz0, rz1 = world_bounds(ring_objs)
...
print(f"Built experiment 16 v01: {len(all_meshes)} elements ({len(PIECES)} timber pieces + raft)")
