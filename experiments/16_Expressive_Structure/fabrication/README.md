...
| `fab_hlr.py` | | exact hidden-line removal for convex, non-intersecting solids in any orthographic view; draws the nine axonometrics. Needs numpy |
...
| `fab_qr.py` | | QR encoder for the viewer link in the title block (byte mode, level L, versions 1 to 5) |
| `svg/` | `fab_drawings.py` | the same sheets for a quick look in a browser |

`fab_cutlist.py` needs only the Python standard library. `fab_drawings.py` also needs numpy, for the axonometrics.
...
- The axonometrics use `fab_hlr.py`: every edge is clipped against every face
...
- Length labels sit at the piece centre (upright pieces 15 % above it, and
  ...
  side of its piece that covers less drawing, with no test against other
  lollipops.
