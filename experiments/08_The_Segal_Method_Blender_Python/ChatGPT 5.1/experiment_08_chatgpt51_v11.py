# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 08 - CHATGPT 5.1 - V11
# THE SEGAL METHOD
# ------------------------------------------------------------------
...
def build_walls():
    ...
    empty cavity between them. Outer panels now meet exactly at the
    floor-1 level (no overlap between ground and upper panels).
    """
    ...
    # Outer lower – from ground to first-floor level (no overlap)
    ...
    for side, y_base in (("S", 0.0), ("N", Ly)):
        ...
        for i in range(NX):
            ...
            # --- OUTER PANELS + SLATS (ground & upper meet at FLOOR1_BOTTOM) ---
            if not (has_door or has_gwin):
                make_box(
...
def build_skylight():
    ...
    The whole skylight assembly is lowered slightly so rafters sit
    comfortably on the south beam; the north beam sits just below the
    rafters, not intersecting them.
    """
    ...
    # slightly lowered (0.02) so rafters sit comfortably into the support
    z_low = ROOF_BEAM_LEVEL + BEAM_HEIGHT / 2.0 - 0.02
    ...
    # Heavy north beam along high edge of roof, located just under
    # the rafters (no intersection) and narrowed to regular beam width.
    ...
    # Put beam top slightly below rafter bottom at north side
    beam_top = z_north_bottom_raf - 0.02
    north_beam_depth = BEAM_HEIGHT * SKY_NORTH_BEAM_FACTOR
    z_beam_center = beam_top - north_beam_depth / 2.0
