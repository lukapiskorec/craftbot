# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 08 - CHATGPT 5.1 - V04
# THE SEGAL METHOD
# ------------------------------------------------------------------
...
# Wall build-up
WALL_TOTAL_THICK = 0.12  # overall ideal wall zone (unused directly)
...
# Ground-floor windows (full bay openings for now)
GROUND_WINDOWS = {
    "S": [2, 5],
}
...
def build_beam_layer(z_center, label):
    ...
    for i in range(NX + 1):
        x = i * MODULE
        for j in range(NY):
            # Skip specific beams at upper-floor level to avoid stair clash
            if label == "FL1" and j == 1 and i in (5, 6):
                continue
...
def build_floor1():
    ...
    # East half (over double-height zone)
    if VOID_SIDE != "E":
        make_box(
            name="Floor1_Deck_E",
...
def build_walls():
    ...
    Openings for doors and windows are implemented by omitting
    both inner and outer panels in designated bays.
    """

    # Inner lower storey: from top of ground floor deck to underside of upper-floor beams
    ...
    # Inner upper storey: from top of upper floor deck to underside of roof beams
    ...
    # Outer lower: from underside of ground floor deck to underside of upper-floor beams
    ...
    # Outer upper: from underside of upper-floor deck to underside of roof beams
...
# ------------------------------------------------------------------
# 6. STAIRCASE (shifted in -X and up against north wall)
...
def build_staircase():
    ...
    positioned against the north wall.
    """
    ...
    inner_face_y = Ly - POST_SECTION / 2.0 - PANEL_THICK  # inside face of inner panel
