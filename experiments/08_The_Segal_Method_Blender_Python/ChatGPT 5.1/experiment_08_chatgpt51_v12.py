# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 08 - CHATGPT 5.1 - V12
# THE SEGAL METHOD
# ------------------------------------------------------------------
...
def build_walls():
    ...
    storeys now meet exactly at FLOOR1_BOTTOM (no overlap).
    """
...
def build_window_frames():
    ...
    # SOUTH & NORTH (vary X) – keep windows for each floor separate
    for side, y_base in (("S", 0.0), ("N", Ly)):
        y_mid = y_base
...
# ------------------------------------------------------------------
# 8. SKYLIGHT ROOF OVER DOUBLE-HEIGHT SPACE
...
def build_skylight():
    ...
    # Low support level: top of south roof beams, slightly lowered
    ...
    # North heavy beam – just under rafters at high side
    ...
    # South beam – square-ish profile sitting on top of south wall,
    # directly under rafter low ends, no vertical extensions.
    z_south_top = z_low
    z_beam_south_center = z_south_top - BEAM_HEIGHT / 2.0
    make_box(
        name="SkylightSouthBeam",
        center=(beam_cx, 0.0, z_beam_south_center),
        size=(beam_span_x, BEAM_WIDTH, BEAM_HEIGHT),
        coll_kind="structure",
    )
