# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 08 - CHATGPT 5.1 - V07
# THE SEGAL METHOD
# ------------------------------------------------------------------
...
# Window / door framing
FRAME_MARGIN = 0.05   # horizontal margin from opening edges
...
def build_walls():
    ...
    # Outer lower (now extended up to top of floor1 beams)
...
def build_window_frames():
    ...
    (column line), with bottoms sitting on the floor and special
    handling for double-height windows.
    """
    ...
    for side, x_base in (("W", 0.0), ("E", Lx)):
        ...
        for j in range(NY):
            ...
            # No tall combined window on E; we keep door + upper tall-ish window
            # Ground openings
            if has_door:
                add_door_frame_ew(
...
def add_window_frame_sn(name_prefix, cx, y, z_bottom, z_top):
    """Window frame for S/N facades (span along X), with mullions."""
    # vertical margins: bottom 0, top FRAME_MARGIN
    open_height = z_top - z_bottom
    open_width = MODULE

    width = open_width - 2 * FRAME_MARGIN
    height = open_height - FRAME_MARGIN
