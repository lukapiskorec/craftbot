# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 08 - CHATGPT 5.1 - V08
# THE SEGAL METHOD
# ------------------------------------------------------------------
...
WINDOW_SILL_CLEAR = 0.05   # sill above finished floor
WINDOW_HEAD_CLEAR = 0.15   # clearance below beam/ceiling

DOOR_SILL_CLEAR = 0.00     # door threshold at floor
...
def build_window_frames():
    ...
    (column line), with bottoms sitting on the floor and a head
    clearance below the beams. Double-height windows have a transom
    aligned with the intermediate floor beam.
    """
    ...
    for side, x_base in (("W", 0.0), ("E", Lx)):
        ...
        for j in range(NY):
            ...
            if has_uwin:
                if in_dh:
                    zb = FLOOR1_BOTTOM       # start at beam level (no slab)
                    zt = upper_inner_z_top
                else:
...
def build_skylight():
    ...
    sawtooth/skylight arrangement.
    """

    # Common rotation (tilt about X so roof slopes in Y)
    euler = (math.radians(SKY_SLOPE_DEG), 0.0, 0.0)

    # Position roof planes just above the roof beams
    base_z = ROOF_BEAM_LEVEL + BEAM_HEIGHT / 2.0 + SKY_ROOF_THICK

    # One inclined roof plane per X bay over the east half
    for i in range(NX // 2, NX):
        cx = (i + 0.5) * MODULE
        make_box(
            name=f"SkylightRoof_{i}",
            center=(cx, Ly / 2.0, base_z),
            size=(MODULE, Ly + POST_SECTION, SKY_ROOF_THICK),
            coll_kind="structure",
            euler=euler,
        )

        # Rafters within this bay, running along Y (parallel to slope)
        for k in range(SKY_RAFTERS_PER_BAY):
            # distribute rafters across bay width in X
            offset = (k + 0.5) * MODULE / SKY_RAFTERS_PER_BAY - MODULE / 2.0
            rx = cx + offset
            make_box(
                name=f"SkylightRafter_{i}_{k}",
                center=(rx, Ly / 2.0, base_z - SKY_ROOF_THICK),
                size=(SKY_RAFTER_WIDTH, Ly + POST_SECTION, SKY_RAFTER_HEIGHT),
                coll_kind="structure",
                euler=euler,
            )
