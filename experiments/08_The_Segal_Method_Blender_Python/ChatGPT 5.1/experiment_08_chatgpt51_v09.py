# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 08 - CHATGPT 5.1 - V09
# THE SEGAL METHOD
# ------------------------------------------------------------------
...
WINDOW_HEAD_CLEAR = 0.15  # clearance below beam/ceiling

DOOR_SILL_CLEAR = 0.0     # door threshold at floor
DOOR_HEAD_CLEAR = 0.10    # clearance below beam
...
SKY_SLOPE_DEG = 15.0        # gentler pitch
SKY_ROOF_THICK = 0.05
SKY_RAFTER_WIDTH = 0.05
SKY_RAFTER_HEIGHT = 0.15
SKY_RAFTERS_PER_BAY = 3
...
def build_window_frames():
    ...
    (column line), with bottoms sitting on the floor and heads below
    the beams above (no intersections). Double-height windows on the
    south are continuous with a transom at the intermediate floor.
    """
...
#    (low edge bearing on south roof beams, high edge on a heavy
#     north beam supported by post extensions)

def build_skylight():
    """
    Adds a series of inclined roof planes and rafters above the
    double-height space (east half of the plan), forming a simple
    north-light arrangement. The low edge bears on the existing roof
    beams along the south wall. A heavy north beam carries the high
    edge, with post extensions underneath.
    """

    theta = math.radians(SKY_SLOPE_DEG)
    span_y = Ly + POST_SECTION
    span_x = (NX - NX // 2) * MODULE + POST_SECTION  # east half + edge

    # Low edge (south) sits on top of existing roof beams:
    z_low = ROOF_BEAM_LEVEL + BEAM_HEIGHT / 2.0

    # For a box with local extents Y: ±span_y/2, Z: ±SKY_ROOF_THICK/2,
    # rotated about X by theta, the south-bottom corner has local z:
    local_z_low_corner = (-span_y / 2.0) * math.sin(theta) + (-SKY_ROOF_THICK / 2.0) * math.cos(theta)

    # Choose world centre so that this south-bottom corner is at z_low
    z_center = z_low - local_z_low_corner
    euler = (theta, 0.0, 0.0)

    # Roof planes and rafters (one set per X-bay over east half)
    for i in range(NX // 2, NX):
        cx = (i + 0.5) * MODULE

        # Roof plane
        make_box(
            name=f"SkylightRoof_{i}",
            center=(cx, Ly / 2.0, z_center),
            size=(MODULE, span_y, SKY_ROOF_THICK),
            coll_kind="structure",
            euler=euler,
        )

        # Rafters within this bay (running along Y, matching slope)
        for k in range(SKY_RAFTERS_PER_BAY):
            offset = (k + 0.5) * MODULE / SKY_RAFTERS_PER_BAY - MODULE / 2.0
            rx = cx + offset
            make_box(
                name=f"SkylightRafter_{i}_{k}",
                center=(rx, Ly / 2.0, z_center - SKY_ROOF_THICK),
    ...
    # Local z of north-top corner of roof box:
    local_z_north_top = (span_y / 2.0) * math.sin(theta) + (SKY_ROOF_THICK / 2.0) * math.cos(theta)
    z_north_top = z_center + local_z_north_top

    north_beam_depth = BEAM_HEIGHT * SKY_NORTH_BEAM_FACTOR
    z_beam_center = z_north_top - north_beam_depth / 2.0

    beam_span_x = span_x
    beam_cx = (NX // 2 * MODULE + NX * MODULE) / 2.0
