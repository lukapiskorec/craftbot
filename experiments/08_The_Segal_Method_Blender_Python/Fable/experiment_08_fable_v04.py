...
def wall_bay(wall, tag, a0, a1, storey, holes=(), floor_strip=True, core=True, trim=(0.0, 0.0)):
    ...
    if floor_strip:
        wall.piece(f"ExtFloor_{tag}", sub("Cladding_Ext"), a0 + T, a1 - T, *wall.across(*LAYERS["ext"]), z_fr, z_sole)
    # clamping battens on the bands, outside and inside, skipping holes
    for k, (b0, b1) in enumerate(bd):
        h = hole_at(b0, b1)
        for za, zb in zrows(z_sole, z_top, h):
            wall.piece(f"BatExt_{tag}_{k}_{za:.2f}", sub("Battens"), b0, b1, *wall.across(*LAYERS["bat_ext"]), za, zb)
        for za, zb in zrows(z_int, z_top, h):
...
print("Experiment 08 Fable v04: members =", len([o for o in bpy.data.objects if o.type == "MESH"]))
