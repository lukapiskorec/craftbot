...
def prism(name, coll, origin, u, v, pts, t0, t1):
    ...
    o = Vector(origin)
    # orient the polygon counter-clockwise about n so all faces point outwards
...
# Gravel oversite (40 mm) between the pads, no concrete slab (fig 33);
# laid as 3 x 3 m fields cut around the pad slabs.
pad_holes = [(x - PAD / 2, x + PAD / 2, sum(post_y_range(y)) / 2 - PAD / 2, sum(post_y_range(y)) / 2 + PAD / 2)
             for x, y in post_xy]
GRAVEL_T = 0.04
STEP_X = (12.40, 13.30)
for k in range(1, 5):
    for xs in (STEP_X[0] + 0.05, STEP_X[1] - 0.10):
        pad_holes.append((xs - 0.125, xs + 0.175, -0.275 * k, -0.275 * k + 0.25))
for i, (ca, cb, ba, bb) in enumerate(tile(-0.9, 15.2, -1.7, 8.7, 3.0, 3.0, pad_holes, stagger=False)):
    box(f"Gravel_{i:03d}", "Foundation", ca, cb, ba, bb, -GRAVEL_T, 0.0)
...
def beam_xs(fi, level):
    """x-ranges of the beams of frame fi: a beam on each post face at every
    frame (v05: end frames too, so end posts are loaded concentrically and
    the gable outriggers bear on the outer beam)."""
    x = FRAMES_X[fi]
...
SKYLIGHTS = [
    (0.475, 1.675, ALL_BANDS[1][1], ALL_BANDS[2][0]),
    (2.175, 3.375, ALL_BANDS[1][1], ALL_BANDS[2][0]),
    (0.475, 1.675, ALL_BANDS[3][1], ALL_BANDS[4][0]),
    (2.175, 3.375, ALL_BANDS[3][1], ALL_BANDS[4][0]),
...
# ridge capping: two boards on the deck meeting plumb at the ridge
RIDGE_CAP = 0.15
roof_piece("RidgeCap_S", "Roof/Fascia", X_ROOF0, X_ROOF1, Y_RIDGE - RIDGE_CAP, Y_RIDGE, zd, lambda y: zd(y) + FASCIA)
roof_piece("RidgeCap_N", "Roof/Fascia", X_ROOF0, X_ROOF1, Y_RIDGE, Y_RIDGE + RIDGE_CAP, zd, lambda y: zd(y) + FASCIA)
# verge beams: 50 thick, full slab depth, tie the outrigger ends together
VERGE = 0.05
for tag, x0, x1 in (("W", X_ROOF0 - VERGE, X_ROOF0), ("E", X_ROOF1, X_ROOF1 + VERGE)):
    roof_piece(f"Verge_{tag}_S", "Roof/Fascia", x0, x1, Y_ROOF0 - FASCIA, Y_RIDGE, zu, zd)
    roof_piece(f"Verge_{tag}_N", "Roof/Fascia", x0, x1, Y_RIDGE, Y_ROOF1 + FASCIA, zu, zd)
...
RAIL_H = 0.90           # horizontal clamping rail at sill height above the sole plate
TILE = 0.30


def wall_bay(wall, tag, a0, a1, storey, holes=(), floor_strip=True, core=True, trim=(0.0, 0.0), tiles=False):
    ...
    sub = lambda layer: f"{coll}/{wall.name}_{layer}"
    sl = slots(a0, a1)
    bd = bands(a0, a1)
    if storey == "G":
    ...
    for h in H:
        if h["z0"] <= z_int + 1e-6:
            ...
            sole_ranges = new
    for ra, rb in sole_ranges:
        wall.piece(f"Sole_{tag}_{n}", sub("Battens"), ra, rb, *wall.across(0.0, CORE), z_sole, z_sole + T)
        n += 1
    z_rail = z_sole + RAIL_H

    def split_rail(rows):
        out = []
        for za, zb in rows:
            if za + 1e-6 < z_rail and (zb is None or z_rail < zb - 1e-6):
                out += [(za, z_rail), (z_rail, zb)]
            else:
                out.append((za, zb))
        return out

    def tile_rows(j, sa0, sa1, za, zb):
        """Tile-hung cladding: 300 x 300 tiles, rows staggered half a tile,
        the last row follows the roof."""
        ext = wall.across(*LAYERS["ext"])
        flat_top = zb if zb is not None else min(wall.ztop(sa0, *ext), wall.ztop(sa1, *ext))
        z, row = za, 0
        while z < flat_top - 1e-6:
            zt = z + TILE
            last = zt >= flat_top - 1e-6
            seams = [sa0 + TILE / 2, sa0 + 1.5 * TILE] if row % 2 else [sa0 + TILE, sa0 + 2 * TILE]
            cols = [sa0] + [c for c in seams if sa0 + 1e-6 < c < sa1 - 1e-6] + [sa1]
            for ci in range(len(cols) - 1):
                wall.piece(f"Tile_{tag}_{j}_{za:.2f}_{row}_{ci}", sub("Cladding_Ext"), cols[ci], cols[ci + 1], *ext,
                           z, (None if (last and zb is None) else min(zt, flat_top)))
            z, row = zt, row + 1

    # exterior ply: storey panels per slot, split at the rail; tiles on gables
    for j, (sa0, sa1) in enumerate(sl):
        h = hole_at(sa0, sa1)
        for za, zb in split_rail(zrows(z_sole, z_top, h)):
            if tiles:
                tile_rows(j, sa0, sa1, za, zb)
            else:
                wall.piece(f"Ext_{tag}_{j}_{za:.2f}", sub("Cladding_Ext"), sa0, sa1, *wall.across(*LAYERS["ext"]), za, zb)
        # horizontal rail outside at sill height where the slot is solid there
        if not (h and h["z0"] < z_rail + T and (h["z1"] is None or h["z1"] > z_rail)):
            wall.piece(f"Rail_{tag}_{j}", sub("Battens"), sa0, sa1, *wall.across(*LAYERS["bat_ext"]), z_rail, z_rail + T)
        for za, zb in zrows(z_core, z_top, h):
            if core:
                wall.piece(f"Core_{tag}_{j}_{za:.2f}", sub("Core"), sa0, sa1, *wall.across(*LAYERS["core"]), za, zb)
        ia0 = sa0 + trim[0] if j == 0 else sa0
        ia1 = sa1 - trim[1] if j == len(sl) - 1 else sa1
        for za, zb in zrows(z_int, z_top, h):
            wall.piece(f"Int_{tag}_{j}_{za:.2f}", sub("Cladding_Int"), ia0, ia1, *wall.across(*LAYERS["int"]), za, zb)
    if floor_strip:
        wall.piece(f"FloorBand_{tag}", sub("Battens"), a0 + T, a1 - T, *wall.across(*LAYERS["ext"]), z_fr, z_sole)
    ...
    for k, (b0, b1) in enumerate(bd):
        ...
        for za, zb in zrows(z_sole, z_top, h):
            parts = [(za, z_rail), (z_rail + T, zb)] if za < z_rail and (zb is None or zb > z_rail + T) else [(za, zb)]
            for ra, rb in parts:
                wall.piece(f"BatExt_{tag}_{k}_{ra:.2f}", sub("Battens"), b0, b1, *wall.across(*LAYERS["bat_ext"]), ra, rb)
        for za, zb in zrows(z_int, z_top, h):
            wall.piece(f"BatInt_{tag}_{k}_{za:.2f}", sub("Battens"), b0, b1, *wall.across(*LAYERS["bat_int"]), za, zb)
    ...
    for hi, h in enumerate(H):
        ...
        for b0, b1 in bd:
            if b0 > ha0 + 1e-6 and b1 < ha1 - 1e-6:
                ...
                cols += [b0, b1]
        cols.append(ha1 - T)
        # transoms on fixed levels (sill rail + 1.2 m steps) so they line up
        # with window sills / heads; panes stay <= 0.6 x 1.5 m (~0.9 m2)
        for ci in range(0, len(cols), 2):
            ...
            if h["kind"] != "door":
                ...
                while zt < tm - 0.3:
                    if zt > hz0 + T + 0.3:
                        wall.piece(f"Transom_{tag}_{hi}_{ci}_{zt:.2f}", wc, ca, cb, *lc, zt - T / 2, zt + T / 2)
...
# South wall: glazed double-height corner bay (S1), glazed ground floor (S2)
wall_bay(S_WALL, "S1G", *bay_x[0], "G", GLAZED_G, trim=(PLY, 0.0))
wall_bay(S_WALL, "S1U", *bay_x[0], "U", GLAZED_U, trim=(PLY, 0.0))
wall_bay(S_WALL, "S2G", *bay_x[1], "G", GLAZED_G)
wall_bay(S_WALL, "S2U", *bay_x[1], "U", [WIN(1, 4, 4.50, 5.70)])
wall_bay(S_WALL, "S3G", *bay_x[2], "G", [WIN(0, 2, 1.90, 3.10), DOOR(4, 5, FFL_G, 3.15)], trim=(0.0, PLY))
...
wall_bay(N_WALL, "N2U", *bay_x[1], "U", [WIN(1, 4, 4.50, 5.70)])
wall_bay(N_WALL, "N3G", *bay_x[2], "G", [], core=False, trim=(0.0, PLY))
wall_bay(N_WALL, "N3U", *bay_x[2], "U", [], core=False, trim=(0.0, PLY))
# West gable: glazed corner (W1 both storeys), tile-hung upper storey on W2
wall_bay(W_WALL, "W1G", *bay_y[0], "G", GLAZED_G, floor_strip=False)
wall_bay(W_WALL, "W1U", *bay_y[0], "U", GLAZED_U, floor_strip=False)
wall_bay(W_WALL, "W2G", *bay_y[1], "G", [WIN(1, 2, 1.90, 3.10)], floor_strip=False)
wall_bay(W_WALL, "W2U", *bay_y[1], "U", [WIN(1, 4, 4.50, 5.70)], floor_strip=False, tiles=True)
# East gable (floor zone = exposed double beam of frame 11.7), tile-hung upper storey
wall_bay(E_WALL, "E1G", *bay_y[0], "G", GLAZED_G, floor_strip=False)
wall_bay(E_WALL, "E1U", *bay_y[0], "U", [WIN(1, 4, 4.50, 5.70)], floor_strip=False, tiles=True)
wall_bay(E_WALL, "E2G", *bay_y[1], "G", [WIN(1, 3, 1.90, 3.10)], floor_strip=False)
wall_bay(E_WALL, "E2U", *bay_y[1], "U", [WIN(1, 4, 4.50, 5.70)], floor_strip=False, tiles=True)

# Canopy over the south door (cover photo): 6 mm sheet on two raking arms
# bolted to the posts / battens, each propped by a strut
CAN_Y0, CAN_Y1 = -POST_OUT - 0.005, -1.05
CAN_Z0, CAN_Z1 = 3.40, 3.20
CAN_W = 0.10
sl3 = slots(*bay_x[2])
arm_xs = (sl3[4][0] - T, bay_x[2][1] - T)
cd = Vector((CAN_Y1 - CAN_Y0, CAN_Z1 - CAN_Z0)).normalized()
cn = Vector((cd.y, -cd.x))          # upward normal of the canopy plane
for k, ax in enumerate(arm_xs):
    prism_x(f"CanopyArm_{k}", "Facade/South/South_Windows", ax, ax + T, strip((CAN_Y0, CAN_Z0), (CAN_Y1, CAN_Z1), CAN_W))
    strut = strip((CAN_Y0, CAN_Z0 - 0.75), (CAN_Y1 + 0.25, CAN_Z1), 0.05, ext=0.3)
    strut = clip(strut, (CAN_Y0 - cn.x * CAN_W / 2, CAN_Z0 - cn.y * CAN_W / 2), (-cn.x, -cn.y))   # below the arm underside
    strut = clip(strut, (CAN_Y0, 0.0), (-1.0, 0.0))                               # outside the post face
    prism_x(f"CanopyStrut_{k}", "Facade/South/South_Windows", ax, ax + T, strut)
off = cn * (CAN_W / 2 + GLASS / 2)
sheet = [(q[0] + off.x, q[1] + off.y) for q in strip((CAN_Y0, CAN_Z0), (CAN_Y1 - 0.05, CAN_Z1), GLASS)]
prism_x("CanopyGlass", "Facade/South/South_Windows", arm_xs[0] - 0.05, arm_xs[1] + T + 0.05, sheet)
...
print("Experiment 08 Fable v06: members =", len([o for o in bpy.data.objects if o.type == "MESH"]))
