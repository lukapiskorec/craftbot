...
def wall_bay(wall, tag, a0, a1, storey, holes=(), floor_strip=True, core=True, trim=(0.0, 0.0), tiles=False):
    ...
    def tile_rows(j, sa0, sa1, za, zb):
        ...
        while z < flat_top - 1e-6:
            ...
            for ci in range(len(cols) - 1):
                wall.piece(f"Tile_{tag}_{j}_{row}_{ci}", sub("Cladding_Ext"), cols[ci], cols[ci + 1], *ext,
    ...
    for hi, h in enumerate(H):
        ...
        for ci in range(0, len(cols), 2):
            ...
            hgt = tm - (hz0 + T)
            nrows = 1 if h["kind"] == "door" else max(1, math.ceil(hgt / 1.25))
            levels = [hz0 + T]
...
print("Experiment 08 Fable v05: members =", len([o for o in bpy.data.objects if o.type == "MESH"]))
