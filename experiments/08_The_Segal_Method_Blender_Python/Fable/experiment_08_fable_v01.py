...
def roof_profile(y0, y1, lo, hi):
    """Plumb-cut (y, z) profile between two roof-parallel height functions.
    A piece straddling the ridge gets a flat bottom and a peaked top so it
    stays convex."""
    if y0 < Y_RIDGE - 1e-6 < y1:
        zb = min(lo(y0), lo(y1))
        return [(y0, zb), (y1, zb), (y1, hi(y1)), (Y_RIDGE, hi(Y_RIDGE)), (y0, hi(y0))]
...
def floor_bay(fi, lvl, z0, band_filter=lambda b: True):
    ...
    box(f"Bearer_{lvl}_B{fi}_W", coll, xa, xa + BEARER, 0.0, Y_OUT, z0, z0 + BEARER)
    box(f"Bearer_{lvl}_B{fi}_E", coll, xb - BEARER, xb, 0.0, Y_OUT, z0, z0 + BEARER)
    for k, (y0, y1) in enumerate(ALL_BANDS):
        if not band_filter((y0, y1)):
            continue
...
for side, y0, y1, flip in (("S", Y_ROOF0, Y_RIDGE, False), ("N", Y_RIDGE, Y_ROOF1, True)):
    cells = tile(X_ROOF0, X_ROOF1, 0.0, y1 - y0, SHEET_A, SHEET_B, [
        (hx0, hx1, (Y_RIDGE - hy1 if flip else hy0 - y0), (Y_RIDGE - hy0 if flip else hy1 - y0))
...
def wall_bay(wall, tag, a0, a1, storey, holes=(), floor_strip=True, core=True):
    ...
    for j, (sa0, sa1) in enumerate(sl):
        ...
        for za, zb in zrows(z_int, z_top, h):
            wall.piece(f"Int_{tag}_{j}_{za:.2f}", coll + "/Cladding_Int", sa0, sa1, *wall.across(*LAYERS["int"]), za, zb)
    if floor_strip:
    ...
    for hi, h in enumerate(H):
        ...
        wc = coll + "/Windows"
        wall.piece(f"Jamb_{tag}_{hi}_a", wc, ha0, ha0 + T, *lc, hz0, hz1)
        wall.piece(f"Jamb_{tag}_{hi}_b", wc, ha1 - T, ha1, *lc, hz0, hz1)
...
GLAZED_G = [dict(j0=0, j1=5, z0=Z_GF + BEAM_D, z1=Z_UF, kind="glazed")]
GLAZED_U = [dict(j0=0, j1=5, z0=Z_UF + BEAM_D, z1=None, kind="glazed")]
...
# South wall
wall_bay(S_WALL, "S1G", *bay_x[0], "G", GLAZED_G)
wall_bay(S_WALL, "S1U", *bay_x[0], "U", [WIN(1, 4, 4.50, 5.70)])
wall_bay(S_WALL, "S2G", *bay_x[1], "G", GLAZED_G)
wall_bay(S_WALL, "S2U", *bay_x[1], "U", GLAZED_U)
wall_bay(S_WALL, "S3G", *bay_x[2], "G", [WIN(0, 2, 1.90, 3.10), DOOR(4, 5, Z_GF + BEAM_D, 3.10)])
wall_bay(S_WALL, "S3U", *bay_x[2], "U", [WIN(1, 4, 4.50, 5.70)])
# North wall (bay 3 braced: no core, no windows)
wall_bay(N_WALL, "N1G", *bay_x[0], "G", [WIN(1, 2, 1.90, 3.10)])
wall_bay(N_WALL, "N1U", *bay_x[0], "U", [WIN(2, 3, 4.50, 5.70)])
wall_bay(N_WALL, "N2G", *bay_x[1], "G", [WIN(1, 4, 1.90, 3.10)])
wall_bay(N_WALL, "N2U", *bay_x[1], "U", [WIN(1, 4, 4.80, 5.70)])
wall_bay(N_WALL, "N3G", *bay_x[2], "G", [], core=False)
wall_bay(N_WALL, "N3U", *bay_x[2], "U", [], core=False)
# West gable
...
for k in (1, 5, 9, 13):
    ...
    box(f"StairBaluster_{k:02d}", "Balustrade", x0, x0 + T, y0, y1, zt - 0.25, z_rail_under)

# upper-floor balustrade on the void edges
def balustrade(prefix, along, a0, a1, c0, c1, z_floor, axis):
    for k, (b0, b1) in enumerate(bands(a0, a1)):
        if axis == "x":
            box(f"{prefix}_Post_{k}", "Balustrade", b0, b1, c0, c1, z_floor, z_floor + 1.0)
        else:
            box(f"{prefix}_Post_{k}", "Balustrade", c0, c1, b0, b1, z_floor, z_floor + 1.0)
    for name, zr in (("Top", (z_floor + 0.95, z_floor + 1.05)), ("Mid", (z_floor + 0.45, z_floor + 0.50))):
        if axis == "x":
            box(f"{prefix}_Rail_{name}", "Balustrade", a0, a1, c0, c1, *zr)
        else:
            box(f"{prefix}_Rail_{name}", "Balustrade", c0, c1, a0, a1, *zr)
...
print("Experiment 08 Fable v01: members =", len([o for o in bpy.data.objects if o.type == "MESH"]))
