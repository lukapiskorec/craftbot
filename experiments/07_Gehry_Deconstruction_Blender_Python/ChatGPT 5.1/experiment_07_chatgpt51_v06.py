# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 07 - CHATGPT 5.1 - V06
...
# (plan amplitudes increased for stronger in/out lean)
...
WEST_EXTRA_Z = 0.30
WEST_OFF_AMP = 1.00  # larger amplitude, will be used with negative sign
...
def build_tessellation_west(corners, corner_tops):
    ...
    for i in range(1, n - 1):
        ...
        # rotated 90° about axis: swap width / thickness relative to studs
        add_segment_between_points(
            f"Tess_W_{i:02d}",
            wall_pt,
            roof_pt,
...
# ---------------------------------------------------------------------------
# FACADE PLYWOOD / BOARD CLADDING (ROTATED 90° AROUND STUD AXIS)
# ---------------------------------------------------------------------------
...
def build_cladding_south(floor_top_z, corners, corner_tops):
    ...
    for i in range(n):
        ...
        # lower board (stud zone) – rotated: width/thickness swapped
        add_segment_between_points(
            f"Clad_S_L_{i:02d}",
            bottom,
            top_wall,
...
def build_cladding_west(floor_top_z, corners, corner_tops):
    ...
    for i in range(n):
        ...
        # rotated 90° around stud axis: swap
        add_segment_between_points(
            f"Clad_W_L_{i:02d}",
            bottom,
            top_wall,
...
def build_gehry_ruled_unit():
    ...
    # Ruled walls with aligned corners and stronger waves
    ...
    # Plywood / board cladding on façades (rotated boards)
