# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 07 - CHATGPT 5.1 - V08
...
# (make them wide enough to visually close the gap – glass-like panels)
TESS_BOARD_WIDTH = STUD_SPACING * 0.9
TESS_BOARD_THICK = PLY_THICK * 0.5
...
CLAD_OFFSET = 0.03                  # offset outward from wall frame

# Roof cover strips – make them wider so they cover the hypar better
ROOF_BOARD_WIDTH = JOIST_SPACING * 1.3
...
# ---------------------------------------------------------------------------
# TESSELLATION PLATES (WALL–ROOF CONNECTORS) – PANELS
# ---------------------------------------------------------------------------
...
def build_tessellation_south(corners, corner_tops):
    ...
    for i in range(1, n - 1):
        ...
        # Single wide panel between wall and roof in this bay
        add_segment_between_points(
            f"Tess_S_{i:02d}",
            wall_pt,
            roof_pt,
...
def build_tessellation_west(corners, corner_tops):
    ...
    for i in range(1, n - 1):
        ...
        # rotate sense (swap board axes) for side walls
        add_segment_between_points(
            f"Tess_W_{i:02d}",
            wall_pt,
            roof_pt,
...
# ---------------------------------------------------------------------------
# FACADE SLATS (SHINGLE / LADDER LOGIC) – VARIABLE HEIGHT
# ---------------------------------------------------------------------------
...
def build_cladding_south_slat(floor_top_z, corners, corner_tops):
    ...
    for i in range(n):
        t = i / max(1, n - 1)
        b = (L * t, 0.0, floor_top_z)
        top = south_top_point(t, corner_tops)
        bottoms.append(b)
        tops.append(top)
        lengths.append(vec_len(vec_sub(top, b)))

    for i in range(n - 1):
        # average stud length in this bay
        avg_len = 0.5 * (lengths[i] + lengths[i + 1])
        slat_height = avg_len / CLAD_SLAT_COUNT
...
def build_cladding_north_slat(floor_top_z, corners, corner_tops):
    ...
    for i in range(n):
        t = i / max(1, n - 1)
        b = (L * t, W, floor_top_z)
        top = north_top_point(t, corner_tops)
        bottoms.append(b)
        tops.append(top)
        lengths.append(vec_len(vec_sub(top, b)))
...
def build_cladding_west_slat(floor_top_z, corners, corner_tops):
    ...
    for i in range(n):
        s_par = i / max(1, n - 1)
        b = (0.0, W * s_par, floor_top_z)
        top = west_top_point(s_par, corner_tops)
        bottoms.append(b)
        tops.append(top)
        lengths.append(vec_len(vec_sub(top, b)))
...
def build_cladding_east_slat(floor_top_z, corners, corner_tops):
    ...
    for i in range(n):
        s_par = i / max(1, n - 1)
        b = (L, W * s_par, floor_top_z)
        top = east_top_point(s_par, corner_tops)
        bottoms.append(b)
        tops.append(top)
        lengths.append(vec_len(vec_sub(top, b)))
...
def organise_collections():
    ...
    Tessellation plates (Tess_*) are treated as cladding, while
    Tess_Stud_* remain in the structural collection.
    """
    ...
    for obj in bpy.data.objects:
        ...
        if name.startswith("Clad_") or name.startswith("Roof_Clad_") \
           or (name.startswith("Tess_") and "_Stud" not in name):
            target = clad_coll
        else:
            target = struct_coll
...
def build_gehry_ruled_unit():
    ...
    # Tessellation panels between walls and roof (glass-like)
    build_tessellation_plates(corners, corner_tops)

    # Shingle-like façade slats, height derived from stud length
