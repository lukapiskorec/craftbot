# ------------------------------------------------------------------
# CRAFT BOT - Experiment 07 (Fable run) - Gehry-style deconstruction - v05
# ------------------------------------------------------------------
...
CAGE_C = Vector((6.60, -1.85, 0.0))      # rooftop lattice cage (chain-link analogue)
CAGE_DX, CAGE_DY, CAGE_H0, CAGE_H1 = 1.20, 1.10, 3.95, 5.20
CAGE_ROT = math.radians(10)
...
# rotated 20 deg in plan, on four vertical legs cut to the roof surface,
# 22 x 45 vertical slats @150 on all faces.
CR = Matrix.Rotation(CAGE_ROT, 3, "Z")
CX, CY = Vector(CR.col[0]), Vector(CR.col[1])
cb = CAGE_BAR


def cage_pt(x, y, z):
    return CAGE_C + CX * x + CY * y + Vector((0, 0, z))


hx, hy = CAGE_DX / 2, CAGE_DY / 2
for lvl, z in enumerate((CAGE_H0, CAGE_H1)):
    # ring of four bars at this level: x-bars full length, y-bars between them
    for sy in (-1, 1):
        fr = Frame(cage_pt(-hx, sy * (hy - cb / 2), z) + Vector((0, 0, -cb / 2)), CX, Vector((0, 0, 1)))
        frame_prism(f"Cage_Ring{lvl}_X{'pn'[sy < 0]}", C_CAGE, fr, [(0, 0), (CAGE_DX, 0), (CAGE_DX, cb), (0, cb)], -cb / 2, cb / 2)
    for sx in (-1, 1):
        fr = Frame(cage_pt(sx * (hx - cb / 2), -hy + cb, z) + Vector((0, 0, -cb / 2)), CY, Vector((0, 0, 1)))
        frame_prism(f"Cage_Ring{lvl}_Y{'pn'[sx < 0]}", C_CAGE, fr, [(0, 0), (CAGE_DY - 2 * cb, 0), (CAGE_DY - 2 * cb, cb), (0, cb)], -cb / 2, cb / 2)
# corner posts between the rings and legs down to the roof boards
for sx in (-1, 1):
    for sy in (-1, 1):
        c = cage_pt(sx * (hx - cb / 2), sy * (hy - cb / 2), 0)
        box_fr = Frame(c + Vector((0, 0, CAGE_H0 + cb / 2)), Vector((0, 0, 1)), CX)
        frame_prism(f"Cage_Post_{'pn'[sx < 0]}{'pn'[sy < 0]}", C_CAGE, box_fr,
                    [(0, -cb / 2), (CAGE_H1 - CAGE_H0 - cb, -cb / 2), (CAGE_H1 - CAGE_H0 - cb, cb / 2), (0, cb / 2)], -cb / 2, cb / 2)
        # leg: from well below up to the bottom ring, cut by the roof top plane at its own (t,u)
        best = min(((S_.P(i / 200, j / 200) - c).to_2d().length, i / 200, j / 200)
                   for S_ in (ROOF_S,) for i in range(0, 201) for j in range(0, 231))
        t_, u_ = best[1], best[2]
        nr = ROOF_S.normal(t_, u_)
        roof_top = (ROOF_S.P(t_, u_) + nr * (GAP + BT + 0.002), nr)
        leg_fr = Frame(c + Vector((0, 0, CAGE_H0 - cb / 2)), Vector((0, 0, -1)), CX)
        frame_prism(f"Cage_Leg_{'pn'[sx < 0]}{'pn'[sy < 0]}", C_CAGE, leg_fr,
                    [(0, -cb / 2), (1.5, -cb / 2), (1.5, cb / 2), (0, cb / 2)], -cb / 2, cb / 2, clips=[roof_top])
# slats: vertical 22 x 45 between the rings, flush with the bar outer faces
z0, z1 = CAGE_H0 + cb / 2, CAGE_H1 - cb / 2
for face, (axis, other, half_a, half_o) in enumerate(((CX, CY, hx, hy), (CY, CX, hy, hx))):
    for sgn in (-1, 1):
        ...
        for k in range(n):
            ...
            c = CAGE_C + axis * a + other * (sgn * (half_o - SLAT_T / 2))
            fr = Frame(c + Vector((0, 0, z0)), Vector((0, 0, 1)), axis)
            frame_prism(f"Cage_Slat_{face}{'pn'[sgn < 0]}_{k:02d}", C_CAGE, fr,
                        [(0, -SLAT_W / 2), (z1 - z0, -SLAT_W / 2), (z1 - z0, SLAT_W / 2), (0, SLAT_W / 2)], -SLAT_T / 2, SLAT_T / 2)
...
print("experiment_07_fable_v05: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
