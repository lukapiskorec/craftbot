# ------------------------------------------------------------------
# CRAFT BOT - Experiment 07 (Fable run) - Gehry-style deconstruction - v06
# ------------------------------------------------------------------
...
CUBE_H = 0.85                 # half size of the tilted cube (1.70 m cube)
CUBE_C = Vector((4.40, -1.95, 3.45))
BRACE_BAYS = (0, 1, -2, -1)   # wall bays with a 47 x 97 diagonal brace
BLOCK_W = 0.070               # eave blocking between rafters over the rail
CAGE_C = Vector((6.47, -1.90, 4.55))     # rooftop lattice cage (chain-link analogue), centre
CAGE_HX, CAGE_HY, CAGE_HZ = 0.70, 0.65, 0.65   # half sizes 1.4 x 1.3 x 1.3
CAGE_EULER = (math.radians(12), math.radians(-8), math.radians(15))   # tilted like the cube
CAGE_BAR, SLAT_T, SLAT_W, SLAT_SP = 0.07, 0.022, 0.045, 0.15
...
def slab_coeffs(fr, p, n, t0, t1):
    ...
    c + ca*a + cb*b >= 0: the plane is shifted by the worst-case thickness
    term, so thick members never pierce it."""
    p, n = Vector(p), Vector(n)
    dn = fr.n.dot(n)
...
def subtract(fr, pts, holes, t0=0.0, t1=0.0):
    ...
    for p, n, cl in holes:
        ...
        rest = clip_lin(rest, -c, -ca, -cb)          # exact complement: pieces never overlap
        if len(rest) < 3:
            break
    return pieces
...
def bar(name, coll, p, q, up, width, depth, w_off=0.0, d_off=0.0, out=None, ext=(0.0, 0.0), clips=()):
    ...
    obj = frame_prism(name, coll, fr, pts, -width / 2, width / 2, clips)
...
# A3 sheathing: the two free facades keep their 9 mm ply; the wrapped
# south and west walls keep it only above the wrap ledger (the strip that
# stays outdoors above the wrap roof); no interior lining anywhere
...
clad("OH_Ply_Ext_S_Upper", C_OH_EXT, "x", 0.0, L, 0.0, PLY_OUT, Z_LEDGE, Z_PANEL1)
clad("OH_Ply_Ext_W_Upper", C_OH_EXT, "y", PLY_OUT, W - PLY_OUT, 0.0, PLY_OUT, Z_LEDGE, Z_PANEL1)
...
box("WR_Ledger_W", C_WR_RAF, X_LEDGE, PLY_OUT, Y_LEDGE, W - PLY_OUT, Z_LEDGE - LEDGE_D, Z_LEDGE)
...
def wrap_wall(tag, S, openings, end_clips):
    ...
    _, e1p, e2p, e3p, cp = bar(f"WR_{tag}_Plate", C_WR_WALL, S.P(0, 0), S.P(1, 0), (0, 0, 1), RAIL_W, P,
                               w_off=SD / 2, d_off=P / 2, out=S.out, clips=end_clips)
    ...
    for i, t in enumerate(ts):
        ...
        stud_clips = clips + (end_clips if i in (0, len(ts) - 1) else [])
        objs, fr, e3 = ruling_member(f"WR_{tag}_Stud_{i:02d}", C_WR_WALL, S, t, 0.0, SD, w0, w1,
                                     clips=stud_clips, holes=holes)
        studs[i] = (fr, e3, w0, w1)
    ...
    # diagonal braces (47 x 97 flat in the stud layer) in the corner and end
    # bays: the leaning walls have no other racking resistance
    nbay = len(ts) - 1
    for j in BRACE_BAYS:
        i = j if j >= 0 else nbay + j
        fr, pts = quad_frame(S, ts[i], ts[i + 1], 0.0, 1.0, f"WR_{tag}_Brace_{i:02d}")
        a, b = (pts[0], pts[2]) if (i + (0 if j >= 0 else 1)) % 2 == 0 else (pts[1], pts[3])
        poly = strip2(a, b, SD, ext=0.5)
        frame_prism(f"WR_{tag}_Brace_{i:02d}", C_WR_WALL, fr, poly, SD / 2 - P / 2, SD / 2 + P / 2,
                    clips=side_planes(i, i + 1) + clips)
    ...
    for k, (ia, ib, extra, kind) in enumerate(openings):
        ...
        for i in range(ia, ib):
            ...
            for j, (p, n) in enumerate(extra):
                ...
                surface_quad(f"WR_{tag}_Op{k}_Trimmer_{j}_{i}", C_WR_WALL, S, ts[i], ts[i + 1], 0.0, 1.0, 0.0, SD - 0.004,
                             clips=band)
            if kind == "window":
        ...
        if kind == "door":
            ...
            surface_quad(f"WR_{tag}_Op{k}_Leaf", C_WR_DOOR, S, ts[ia], ts[ib], 0.0, 1.0,
...
WIN_W_PLANES = [hplane(0, -0.36, 1.60, 0, -0.45, 1.0),     # wedge window: sill falls northwards
                hplane(0, -0.36, 1.70, 0, 0.75, -1.0)]     # head rises northwards (wedge opens)
DOOR_W_PLANES = [hplane(0, 0, Z_DECK + 2.10, 0, 0, -1.0)]  # head 2.10 above the deck
# end half-spaces: members with slanted end faces (cut square to a tilted
# axis) must stay between the corner post face and the wing end plane
POST_FACE_S = (Vector((X_WRAP, 0, 0)), Vector((1, 0, 0)))
POST_FACE_W = (Vector((0, Y_WRAP, 0)), Vector((0, 1, 0)))
END_S = (Vector((L, 0, 0)), Vector((-1, 0, 0)))
END_W = (Vector((0, W, 0)), Vector((0, -1, 0)))
TS_S, STUDS_S, RAILBAR_S = wrap_wall("WallS", WALL_S, [(5, 8, WIN_S_PLANES, "window")], [POST_FACE_S, END_S])
TS_W, STUDS_W, RAILBAR_W = wrap_wall("WallW", WALL_W, [(4, 6, WIN_W_PLANES, "window"),
                                                    (7, 9, DOOR_W_PLANES, "door")], [POST_FACE_W, END_W])
...
def surf_extent(S, planes, clear, depths=(0.0, -RAF_D)):
    """(t, u) bounding box of the surface samples (at the given normal
    depths) that lie inside the convex region `planes` expanded by `clear`."""
...
def wrap_roof(tag, S, rail, end_plane, ledger_plane, eave_h_out, cube=False):
    ...
    ts[-1] = 1.0
    # cube opening: rafters crossing it are cut at two cross-ruling lines
    # (u_lo, u_hi) just outside the cube and carried by trimmers between the
    # nearest uncut rafters (the dormer rule: header between doubled rafters)
    cut = None
    if cube:
        tmin, tmax, umin, umax = surf_extent(S, CUBE_PLANES, CUBE_CLEAR_RAFTER)
        ta = max(t for t in ts if t < tmin)
        tb = min(t for t in ts if t > tmax)
        wu = P / (S.P(0.5, 1) - S.P(0.5, 0)).length
        cut = (ta, tb, umin - 0.02, umax + 0.02, wu)
    rafters = {}
    for i, t in enumerate(ts, start=1):
        w0, w1 = (-P, 0.0) if t == 1.0 else (-P / 2, P / 2)
        holes = ()
        if cut and cut[0] < t < cut[1]:
            ta, tb, ulo, uhi, wu = cut
            # cut planes contain the trimmer faces: the cross ruling at u and
            # the roof normal (not simply perpendicular to the ruling - the
            # two parametric directions of a hypar are not orthogonal)
            holes = []
            for u_, sg in ((ulo - wu, 1), (uhi + wu, -1)):
                m = S.normal(t, u_).cross(S.dt(t, u_).normalized()).normalized()
                if m.dot(S.du(t, u_)) < 0:
                    m = -m
                holes.append((S.P(t, u_), m * sg, 0.003))
        objs, fr, e3 = ruling_member(f"WR_{tag}_Rafter_{i:02d}", C_WR_RAF, S, t, -RAF_D, 0.0, w0, w1,
                                     clips=[ledge, eave] + ([end_plane] if t == 1.0 else []), holes=holes)
        rafters[i] = (fr, e3, w0, w1, t)

    def rafter_sides(i, j):
        fra, e3a, _, w1a = rafters[i][:4]
        frb, e3b, w0b, _ = rafters[j][:4]
        return [(fra.o + e3a * w1a, e3a), (frb.o + e3b * w0b, -e3b)]

    if cut:
        ta, tb, ulo, uhi, wu = cut
        ia = [i for i in rafters if rafters[i][4] == ta][0]
        ib = [i for i in rafters if rafters[i][4] == tb][0]
        cube_holes = [(p, n, CUBE_CLEAR_RAFTER) for p, n in CUBE_PLANES]
        for k, (u0, u1) in enumerate(((ulo - wu, ulo), (uhi, uhi + wu))):
            surface_quad(f"WR_{tag}_Trimmer_{k}", C_WR_RAF, S, ta, tb, u0, u1, -RAF_D, 0.0,
                         clips=rafter_sides(ia, ib), holes=cube_holes)
    # eave blocking between rafters, standing on the rail, up to the boards
    e1r, e2r, e3r, cr = rail
    rail_top = (cr + e3r * (P / 2), e3r)
    idx = sorted(rafters)
    for i, j in zip(idx[:-1], idx[1:]):
        tm = (rafters[i][4] + rafters[j][4]) / 2
        dl = (S.P(tm, 1) - S.P(tm, 0)).length
        surface_quad(f"WR_{tag}_Block_{i:02d}", C_WR_RAF, S, rafters[i][4], rafters[j][4],
                     1 + 0.015 / dl, 1 + (0.015 + BLOCK_W) / dl, -0.35, 0.0, clips=[rail_top] + rafter_sides(i, j))
...
EAVE_S = wrap_roof("RoofS", ROOF_S, RAILBAR_S, END_S, LEDGE_S, (0, -1, 0), cube=True)
EAVE_W = wrap_roof("RoofW", ROOF_W, RAILBAR_W, END_W, LEDGE_W, (-1, 0, 0))
...
# ------------------------------------------------------------------
# PART D - ROOFTOP LATTICE CAGE: the timber analogue of the chain-link
# volumes that rise above the wrap roof.  A box frame of 70 x 70 bars
# tilted about all three axes (like the cube), 22 x 45 slats @150 on all
# four sides, standing on four vertical legs cut between the roof boards
# and the underside plane of the tilted frame.
CR = Matrix.Rotation(CAGE_EULER[2], 3, "Z") @ Matrix.Rotation(CAGE_EULER[1], 3, "Y") @ Matrix.Rotation(CAGE_EULER[0], 3, "X")
CX, CY, CZ = (Vector(CR.col[k]) for k in range(3))
cb = CAGE_BAR
hx, hy, hz = CAGE_HX, CAGE_HY, CAGE_HZ


def cage_pt(x, y, z):
    return CAGE_C + CX * x + CY * y + CZ * z


for lvl, sz in enumerate((-1, 1)):
    zc = sz * (hz - cb / 2)
    for sy in (-1, 1):                     # x-bars full length, corner cells
        fr = Frame(cage_pt(-hx, sy * (hy - cb / 2), zc) - CZ * (cb / 2), CX, CZ)
        frame_prism(f"Cage_Ring{lvl}_X{'pn'[sy < 0]}", C_CAGE, fr, [(0, 0), (2 * hx, 0), (2 * hx, cb), (0, cb)], -cb / 2, cb / 2)
    for sx in (-1, 1):                     # y-bars between them
        fr = Frame(cage_pt(sx * (hx - cb / 2), -hy + cb, zc) - CZ * (cb / 2), CY, CZ)
        frame_prism(f"Cage_Ring{lvl}_Y{'pn'[sx < 0]}", C_CAGE, fr, [(0, 0), (2 * hy - 2 * cb, 0), (2 * hy - 2 * cb, cb), (0, cb)], -cb / 2, cb / 2)
cage_bottom = (CAGE_C - CZ * hz, -CZ)        # keep below the frame underside
for sx in (-1, 1):
    for sy in (-1, 1):
        cx_, cy_ = sx * (hx - cb / 2), sy * (hy - cb / 2)
        fr = Frame(cage_pt(cx_, cy_, -hz + cb), CZ, CX)   # corner post between the rings
        frame_prism(f"Cage_Post_{'pn'[sx < 0]}{'pn'[sy < 0]}", C_CAGE, fr,
                    [(0, -cb / 2), (2 * hz - 2 * cb, -cb / 2), (2 * hz - 2 * cb, cb / 2), (0, cb / 2)], -cb / 2, cb / 2)
        # vertical leg under the corner, cut by the roof top plane at its own (t,u)
        c = cage_pt(cx_, cy_, -hz)
        best = min(((ROOF_S.P(i / 200, j / 200) - c).to_2d().length, i / 200, j / 200)
                   for i in range(0, 201) for j in range(0, 231))
        t_, u_ = best[1], best[2]
        nr = ROOF_S.normal(t_, u_)
        roof_top = (ROOF_S.P(t_, u_) + nr * (GAP + BT + 0.002), nr)
        leg_fr = Frame(Vector((c.x, c.y, c.z + 0.6)), Vector((0, 0, -1)), CX - CX.dot(Vector((0, 0, 1))) * Vector((0, 0, 1)))
        frame_prism(f"Cage_Leg_{'pn'[sx < 0]}{'pn'[sy < 0]}", C_CAGE, leg_fr,
                    [(0, -cb / 2), (2.5, -cb / 2), (2.5, cb / 2), (0, cb / 2)], -cb / 2, cb / 2,
                    clips=[roof_top, cage_bottom])
# slats between the rings, flush with the bar outer faces
for face, (axis, other, half_a, half_o) in enumerate(((CX, CY, hx, hy), (CY, CX, hy, hx))):
    for sgn in (-1, 1):
        run = 2 * half_a - 2 * cb
        n = int(run // SLAT_SP)
        for k in range(n):
            a = -half_a + cb + SLAT_SP * (k + 0.5)
            c = CAGE_C + axis * a + other * (sgn * (half_o - SLAT_T / 2)) + CZ * (-hz + cb)
            fr = Frame(c, CZ, axis)
            frame_prism(f"Cage_Slat_{face}{'pn'[sgn < 0]}_{k:02d}", C_CAGE, fr,
                        [(0, -SLAT_W / 2), (2 * hz - 2 * cb, -SLAT_W / 2), (2 * hz - 2 * cb, SLAT_W / 2), (0, SLAT_W / 2)],
                        -SLAT_T / 2, SLAT_T / 2)
...
for r, nm in sorted(RESIDUALS, reverse=True)[:5]:
    print("out-of-plane residual %.1f mm  %s" % (r * 1000, nm))
print("experiment_07_fable_v06: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
