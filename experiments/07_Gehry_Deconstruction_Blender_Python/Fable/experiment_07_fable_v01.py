# ------------------------------------------------------------------
# CRAFT BOT - Experiment 07 (Fable run) - Gehry-style deconstruction
# ------------------------------------------------------------------
...
LEAN_S, LEAN_W = 0.50, 0.50   # outward lean of the wall tops at their far ends
Z_TOP_C = 2.88                # rail top at the corner post
Z_TOP_S = 3.45                # rail top, south wall east end
Z_TOP_W = 3.74                # rail top, west wall north end
...
def frame_prism(name, coll, fr, pts, t0, t1, clips=()):
    ...
    t1, after clipping by 3D half-spaces [(p, n)].  None if clipped away."""
    for p, n in clips:
        pts = fr.clip(pts, p, n)
        if len(pts) < 3:
            return None
...
def subtract(fr, pts, holes):
    """Convex polygon minus a convex hole given as inside half-spaces
    [(p, n, clearance)]; returns a list of convex pieces."""
    pieces, rest = [], pts
    for p, n, cl in holes:
        p = Vector(p) - Vector(n) * cl
        outside = fr.clip(rest, p, -Vector(n))
        if len(outside) >= 3:
...
def ruling_member(name, coll, S, t, d0, d1, w0, w1, clips=(), ext=0.5, holes=()):
    ...
    for p, nn in clips:
        pts = fr.clip(pts, p, nn)
        if len(pts) < 3:
            return [], fr, e3
    polys = subtract(fr, pts, holes) if holes else [pts]
    objs = []
...
# B4 wrap walls ---------------------------------------------------------
def wrap_wall(tag, S, openings, x_or_y):
    """Studs along rulings, plate and rail as straight bars, tapered
    boards along the rulings.  openings = [(t_a, t_b, extra_planes,
    kind)] with kind 'window' or 'door'; t_a/t_b are stud indices."""
    A, B = S.A0, S.A1
    length = (B - A).length
    up = ((S.du(0, 0.5) + S.du(1, 0.5)) / 2)
    # plate (u = 0 line is its bottom) and rail (u = 1 line is its top)
    ...
    _, e1r, e2r, e3r, cr = bar(f"WR_{tag}_Rail", C_WR_WALL, S.P(0, 1), S.P(1, 1), up, RAIL_W, P,
                               w_off=SD / 2, d_off=-P / 2, out=S.out)
    clips = [(cp + e3p * (P / 2), e3p), (cr - e3r * (P / 2), -e3r)]
    # stud parameters: 610 grid from the corner, last stud at the end
    ts = [c / length for c in positions(0.0, length, M, P)]
    ts[0], ts[-1] = 0.0, 1.0
    # opening planes (inside half-spaces) per opening, keyed by stud index
    studs = {}
    hole_by_stud = {}
    open_planes = []
    for (ia, ib, extra, kind) in openings:
        open_planes.append((ia, ib, extra, kind))
    for i, t in enumerate(ts):
        w0, w1 = (0.0, P) if i == 0 else ((-P, 0.0) if i == len(ts) - 1 else (-P / 2, P / 2))
        holes = []
        for (ia, ib, extra, kind) in open_planes:
            if ia < i < ib:
                holes.append([(p, n, SD) for p, n in extra])     # cut back for the header / sill
        objs, fr, e3 = ruling_member(f"WR_{tag}_Stud_{i:02d}", C_WR_WALL, S, t, 0.0, SD, w0, w1,
                                     clips=clips, holes=holes[0] if holes else ())
        studs[i] = (fr, e3, w0, w1)
    # side planes of the openings = faces of the boundary studs
    def side_planes(ia, ib):
        fra, e3a, _, w1a = studs[ia]
        frb, e3b, w0b, _ = studs[ib]
        return [(fra.o + e3a * w1a, e3a), (frb.o + e3b * w0b, -e3b)]
    # boards along the rulings, nb per bay, tapered, 4 mm joints
    for i in range(len(ts) - 1):
        ...
        for (ia, ib, extra, kind) in open_planes:
            if ia <= i < ib:
                holes.append([(p, n, 0.0) for p, n in side_planes(ia, ib) + list(extra)])
        for k in range(nb):
            t0 = ta + (tb - ta) * k / nb
            t1 = ta + (tb - ta) * (k + 1) / nb
            surface_quad(f"WR_{tag}_Board_{i:02d}_{k}", C_WR_CLAD, S, t0, t1, 0.0, 1.0,
                         SD + GAP, SD + GAP + BT, gap=GAP / 2, holes=holes[0] if holes else ())
    # openings: header / sill bands in the stud layer, glass or door leaf
    for k, (ia, ib, extra, kind) in enumerate(open_planes):
        sides = side_planes(ia, ib)
        for j, (p, n) in enumerate(extra):
            band = [(p, -Vector(n)), (Vector(p) - Vector(n) * SD, n)] + sides
            surface_quad(f"WR_{tag}_Op{k}_Frame_{j}", C_WR_WALL, S, ts[ia], ts[ib], 0.0, 1.0, 0.0, SD,
                         clips=band)
        inside = sides + list(extra)
        if kind == "window":
            for i in range(ia, ib):
                surface_quad(f"WR_{tag}_Op{k}_Glass_{i}", C_WR_OPEN, S, ts[i], ts[i + 1], 0.0, 1.0,
                             SD / 2 - GLASS / 2, SD / 2 + GLASS / 2, clips=inside, gap=0.0)
        else:
            surface_quad(f"WR_{tag}_Op{k}_Leaf", C_WR_OPEN, S, ts[ia], ts[ib], 0.0, 1.0,
                         SD - 0.045, SD - 0.005, clips=inside, gap=0.01)
...
TS_S, STUDS_S, RAILBAR_S = wrap_wall("WallS", WALL_S, [(5, 8, WIN_S_PLANES, "window")], "x")
TS_W, STUDS_W, RAILBAR_W = wrap_wall("WallW", WALL_W, [(4, 6, WIN_W_PLANES, "window"),
                                                    (7, 9, DOOR_W_PLANES, "door")], "y")
...
def wrap_roof(tag, S, rail, other_corner_dir, ledger_plane, eave_h_out, cube=False):
    ...
    for k in range(nb):
        ...
        surface_quad(f"WR_{tag}_Board_{k:02d}", C_WR_RBOARD, S, 0.0, 1.0, u0, u1, GAP, GAP + BT,
...
box("WR_Step_Out_0", C_WR_STEP, x_face - 0.90, x_face, yd - 0.60, yd + 0.60, 0.0, 0.18)
box("WR_Step_Out_1", C_WR_STEP, x_face - 0.60, x_face, yd - 0.60, yd + 0.60, 0.18, 0.36)
...
print("experiment_07_fable_v01: built", len([o for o in bpy.data.objects if o.type == "MESH"]), "members")
