...
#   6 purlins / eave purlins on the principals, ridge on the king posts
...
EAVE_OVERHANG = 0.6                # common rafter tail beyond post centre
PURLIN_Y = 1.7                     # mid-slope purlin position (|y|)
EAVE_PURLIN_Y = 2.8
...
for i, x in enumerate(BENT_X):
    ...
    for side, sgn in (("L", -1), ("R", 1)):
        ...
        s0 = Vector((x, sgn * KING[1] / 2, TIE_TOP + 0.35))
...
for side, sgn in (("L", -1), ("R", 1)):
    for tag, yy in (("Mid", PURLIN_Y), ("Eave", EAVE_PURLIN_Y)):
        z_top = principal_underside_z(yy) + PRINCIPAL[1] / C       # principal top surface
        normal = Vector((0, sgn * S, C))                            # roof-plane normal
        centre = Vector((0, sgn * yy, z_top)) + normal * PURLIN[1] / 2
        p0 = Vector((LONG_X[0], centre.y, centre.z))
        p1 = Vector((LONG_X[1], centre.y, centre.z))
        member(f"Purlin_{tag}_{side}", "Roof_Longitudinal", p0, p1, PURLIN[0], PURLIN[1],
