# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 15 (Koreni), Fable run, v02
#
...
def bar_superstructure(v, floor, x_end, roof_top, closed_concrete=True, x_start=11.0, level_edges=(),
    ...
    ya = y0 + T_CLOSED if s_end > 0 else y0 + POST_IN + md / 2
    yb = y1 - POST_IN - md / 2 if s_end > 0 else y1 - T_CLOSED
...
def build_A():
    ...
    for x, yb_ in ((56.9, void[2]), (60.0, y1 - t)):
        v.partition_x("BoxPart", x, A_BOX_FLOOR, g_bot, y_lo=y0 + t, y_hi=yb_)
        v.partition_x("BoxPart", x, g_bot - PART_GAP, g_top, y_lo=max(y0 + t, gi0), y_hi=min(yb_, gi1), door=False)
    ...
    v.box("WellGuard", "Glazing", x_lo_top, void[1], wg[0], wg[1], z_land, z_land + GUARD_H)
    v.terrain()
    return v
...
def build_B():
    ...
    v.partition_x("Part", 60.0, F, ru)                   # bedroom 3 / master
    v.terrain()
    return v
...
print(f"Built experiment 15 v02: {n_mesh} elements " + ", ".join(f"{k}: {n}" for k, n in sorted(per.items())))
