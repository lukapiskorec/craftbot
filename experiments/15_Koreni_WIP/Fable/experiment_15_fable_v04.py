# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 15 (Koreni), Fable run, v04
#
...
#
# v04: R-45 as rewritten by the Designer (2026-09-10): the clear height at
# the downhill end of a C level is floor to slab underside, at least 3.0 m
# (CLEAR_MIN); CLEAR_H 3.30 stays the roof-height derivation of A and B.
# Closed-wall doors placed where the outside surface is within one riser of
# the floor and clear of the cores and stairs (v03 put one in segment 4's
# last bay of every variation, onto a drop and into core 4's back wall).
#
...
CLEAR_H = 3.30                                  # roof-height derivation (A, B): floor to beam underside
CLEAR_MIN = 3.00                                # C: floor to roof slab underside at a level's downhill end (R-45, LHDG 5.4.1 asks 2.5)
...
DOOR_STEP = 0.165                               # outside surface at most one riser below a closed-wall door threshold (mine, R-18 riser)
...
def bar_superstructure(v, floor, x_end, roof_top, closed_concrete=True, x_start=11.0, level_edges=(),
                       first_post=False, start_parapet=False, outside=None, keep_out=()):
    ...
    outside(x, side): the outside surface level at the closed wall (ground, or
    B's courtyard paving); keep_out: x ranges of the cores and stairs against
    the closed wall, which get no door."""
    floor = fn(floor)
    outside = outside or (lambda x, side: zg(x))
    top = fn(roof_top)
    ...
    v.storefront("GlzEnd", "y", ya, yb, x_house_end - md / 2, floor(x_house_end - 0.5), under(x_house_end),
                 (True, True), door=1)
    # closed walls: floor to roof, beam pockets, one strip window per segment, and a
    # door where the outside surface is within one riser of the floor (R-10: doors
    # and one strip window are the only openings); split at the level edges so
    # every piece has one floor level
    def closed_door(la, lb, zf, side, bays, mid):
        """One door opening in the closed-wall piece la..lb (floor zf) whose
        threshold meets the outside surface flush or with at most one riser
        down at both jambs; the downhill-most bay that is not the window bay,
        0.3 clear of the beams, 0.1 clear of keep_out; [] when nothing fits."""
        for pa, pb in reversed(bays):
            if (pa, pb) == mid or pb <= la + EPS or pa >= lb - EPS:
                continue
            lo = max(pa, la) + bw / 2 + 0.3
            hi = min(pb, lb) - bw / 2 - 0.3 - DOOR_W
            if hi < lo:
                continue
            xg = x_at_ground(zf + 0.02)                         # uphill jamb where the ground reaches the floor
            for d0 in (min(max(xg, lo), hi), hi, lo):
                d1 = d0 + DOOR_W
                level_ok = all(zf - DOOR_STEP - EPS <= outside(x, side) <= zf + 0.02 + EPS for x in (d0, d1))
                clear = not any(k0 - 0.1 < d1 and k1 + 0.1 > d0 for k0, k1 in keep_out)
                if level_ok and clear:
                    print(f"{v.L} closed-wall door at x = {d0:.2f}..{d1:.2f}, floor {zf:+.2f}, outside {outside(d0, side):+.2f}..{outside(d1, side):+.2f}")
                    return [(d0, d1, zf, zf + DOOR_H)]
        return []
    ...
    for x0, x1, s in SEGMENTS:
        ...
        for la, lb in cuts(a, b, level_edges):
            zf = floor((la + lb) / 2)
            door = closed_door(la, lb, zf, -s, bays, mid)
            v.wall_x(fam, sub, la, lb, ya, yb, zf, under, ops + door, breaks=[x + bw / 2 for x in xs] + [x - bw / 2 for x in xs])
...
def build_A():
    ...
    # the bridge floor is never within a riser of the ground, so no closed-wall door
    core_x = [(11.2, 15.2), (40.0, 43.0), (44.2, 47.2), (61.5, 64.5)]        # cores 1 to 4 along x (3.1)
    xs = bar_superstructure(v, F, ROOF_END["A"], roof_top, closed_concrete=False, start_parapet=True, keep_out=core_x)
    terrace_balustrade(v, HOUSE_END["A"], BAR_END["A"], F)
    # cores (3.1): 1 entry (store), 2 kitchen, 3 study, 4 master ensuite, 5 box bathroom
    v.core("Core1", *core_x[0], 10.6, y1 - T_CLOSED, F, "y0", store=True)
    v.core("Core2", *core_x[1], y0 + T_CLOSED, y0 + T_CLOSED + 2.4, F, "y1")
    v.core("Core3", *core_x[2], y1 - T_CLOSED - 2.4, y1 - T_CLOSED, F, "y0")
    v.core("Core4", *core_x[3], y0 + T_CLOSED, y0 + T_CLOSED + 2.4, F, "y1")
...
def build_B():
    ...
    # low bar from the step wall on: superstructure with the roof at +1.23; the closed
    # walls of the cut half get doors onto the courtyard paving (0.15 below the floor)
    core_x = [(11.0, 15.0), (39.0, 42.0), (45.0, 48.0), (63.0, 65.8)]        # cores 1 to 4 along x (4.1)
    court = lambda x, side: pav_court if xw1 - EPS <= x <= x_cut1 + EPS else zg(x)
    bar_superstructure(v, F, x_end, roof_top, closed_concrete=True, x_start=x_step1, first_post=True,
                       outside=court, keep_out=core_x + [(lx0, lx1 + GOING * (n_r - 1))])       # landing and flight
    ...
    v.core("Core1", *core_x[0], 9.2, ly0 - BALU_T, F, "y0", store=True)
    v.core("Core2", *core_x[1], y0 + T_CLOSED, y0 + T_CLOSED + 2.4, F, "y1")
    v.core("Core3", *core_x[2], y1 - T_CLOSED - 2.4, y1 - T_CLOSED, F, "y0")
    v.core("Core4", *core_x[3], y0 + T_CLOSED, y0 + T_CLOSED + 2.4, F, "y1")
...
def build_C():
    ...
    for i, (x0, x1, F) in enumerate(C_LEVELS):
        ...
        check(under(x1) - F >= CLEAR_MIN - EPS, f"C L{i + 1} clear height at its downhill end x = {x1}: {under(x1) - F:.2f} m (>= {CLEAR_MIN}, R-45)")
    # superstructure: the whole bar from x = 11 to 70 under the folded plane R2; the
    # closed walls get a door where a level meets the grade (L2 at x = 44.5, L3 at 56.5)
    core_x = [(11.0, 14.0), (40.0, 43.0), (51.0, 54.0), (60.0, 63.0)]        # cores 1 to 4 along x (5.1)
    stair_x_ranges = [(x0, x0 + GOING * 8) for x0, x1, F in C_LEVELS]        # 9 risers x 280 going (R-42)
    xs = bar_superstructure(v, floor_at, x_end, c_roof_top, closed_concrete=True, level_edges=level_edges,
                            keep_out=core_x + stair_x_ranges)
    ...
    v.core("Core1", *core_x[0], 9.2, y1 - T_CLOSED - STAIR_W - BALU_T, C_LEVELS[0][2], "y0", store=True)
    v.core("Core2", *core_x[1], y0 + T_CLOSED, y0 + T_CLOSED + 2.4, C_LEVELS[1][2], "y1")
    v.core("Core3", *core_x[2], y1 - T_CLOSED - 2.4, y1 - T_CLOSED, C_LEVELS[2][2], "y0")
    v.core("Core4", *core_x[3], y0 + T_CLOSED, y0 + T_CLOSED + 2.4, C_LEVELS[2][2], "y1")
    F3 = C_LEVELS[2][2]
...
print(f"Built experiment 15 v04: {n_mesh} elements " + ", ".join(f"{k}: {n}" for k, n in sorted(per.items())))
