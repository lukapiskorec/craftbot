# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 02 - CHATGPT 5.1 - V03
...
def make_gable_truss_details(index, y, eave_L, eave_R, ridge):
    ...
    - Continuous king post from tie to ridge
    - Diagonals from tie near posts up to collar midpoint (W-like web)
    """
    # Collar a bit lower than before for nicer proportions
    t_collar = 0.55
    ...
    # Continuous king post from tie mid to ridge
    tie_mid = (0.0, y, EAVE_HEIGHT)
    beam_between(f"king_{index}", tie_mid, ridge, BEAM_SIZE * 0.7)

    # W-like diagonals: from tie near the posts up to collar midpoint
    tie_left  = (-HALF_SPAN * 0.6, y, EAVE_HEIGHT)
    tie_right = ( HALF_SPAN * 0.6, y, EAVE_HEIGHT)
    beam_between(f"web_{index}_L", tie_left,  collar_mid, BEAM_SIZE * 0.6)
    beam_between(f"web_{index}_R", tie_right, collar_mid, BEAM_SIZE * 0.6)
