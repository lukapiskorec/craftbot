# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 02 - CHATGPT 5.1 - V04
...
def make_gable_truss_details(index, y, eave_L, eave_R, ridge):
    ...
    - Central king post from tie to ridge
    - Offset vertical post on right (tie -> collar)
    - Diagonals:
        * collar_left  -> bottom of offset vertical
        * tie_mid      -> top of offset vertical
    """
    # Collar roughly mid-way between tie and ridge
    t_collar = 0.50
    ...
    # Central king post (tie -> ridge)
    beam_between(f"king_{index}", tie_mid, ridge, BEAM_SIZE * 0.7)

    # Offset vertical post on the right between tie and collar
    offset_x = HALF_SPAN * 0.35  # position somewhere between centre and right rafter
    # Find z for the collar along its line at that x:
    # linear interpolation along collar beam
    t_offset = (offset_x - collar_L[0]) / (collar_R[0] - collar_L[0])
    z_collar_offset = collar_L[2] + (collar_R[2] - collar_L[2]) * t_offset

    offset_top = (offset_x, y, z_collar_offset)
    offset_bot = (offset_x, y, EAVE_HEIGHT)
    beam_between(f"inner_post_{index}", offset_bot, offset_top, BEAM_SIZE * 0.7)

    # Diagonal from left collar end to bottom of offset vertical
    beam_between(f"diag_{index}_A", collar_L, offset_bot, BEAM_SIZE * 0.6)

    # Diagonal from tie midpoint up to top of offset vertical
    beam_between(f"diag_{index}_B", tie_mid, offset_top, BEAM_SIZE * 0.6)
