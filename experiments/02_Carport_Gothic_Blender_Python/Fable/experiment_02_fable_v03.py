# ------------------------------------------------------------------
# EXPERIMENT 02 - Fable run, v03
# v03: jowls on the post tops (reference detail), post plinths, purlin cleats
#      on every principal, eave purlin moved over the post line (|y| = 2.6).
# v02: longer longitudinal V braces (foot 2.2 m, reach 1.0 m) and section knee
#      braces landing closer to the pendant (|y| = 1.4), to match the reference.
...
POST = (0.24, 0.24)                # x, y
PLINTH = (0.36, 0.10)              # post base: side, height above the slab
JOWL = (0.08, 0.50)                # flare on the post's inner face: projection (y), height
...
LONG_BRACE_REACH = 1.0             # plate contact point from the post centre (x)
LONG_BRACE_FOOT = 2.20             # brace foot on the post (z)
KNEE_FOOT = 1.70                   # section knee brace foot on the post (z)
KNEE_HEAD_Y = 1.40                 # where it meets the hammer beam underside (|y|)
...
PURLIN_Y = [0.9, 1.8, 2.6]         # purlin positions on the slope (|y|); last one over the post line
CLEAT = (0.10, 0.08)               # purlin cleat: width along slope, depth
...
for i, bx in enumerate(BENT_X):
    for s, side in ((-1, "S"), (1, "N")):
        box(f"Plinth_{i + 1}{side}", "Foundation",
            (bx - PLINTH[0] / 2, s * POST_Y - PLINTH[0] / 2, 0.0),
            (bx + PLINTH[0] / 2, s * POST_Y + PLINTH[0] / 2, PLINTH[1]))
        box(f"Post_{i + 1}{side}", "Posts",
            (bx - POST[0] / 2, s * POST_Y - POST[1] / 2, PLINTH[1]),
            (bx + POST[0] / 2, s * POST_Y + POST[1] / 2, POST_H))
        # jowl: the post flares on its inner face under the hammer beam (box instead of a taper)
        y_face = s * (POST_Y - POST[1] / 2)
        box(f"Jowl_{i + 1}{side}", "Posts",
            (bx - POST[0] / 2, min(y_face, y_face - s * JOWL[0]), POST_H - JOWL[1]),
            (bx + POST[0] / 2, max(y_face, y_face - s * JOWL[0]), POST_H))
...
# purlin cleats: a block on the principal's back directly downslope of each purlin
for s, side in ((-1, "S"), (1, "N")):
    for k, py in enumerate(PURLIN_Y):
        cy = py + (PURLIN[0] / 2 + CLEAT[0] / 2) * C
        for i, bx in enumerate(BENT_X):
            slope_member(f"Cleat_{i + 1}{side}{k + 1}", "Purlins",
                         bx - PRINCIPAL[0] / 2, bx + PRINCIPAL[0] / 2, s * cy,
                         CLEAT[0], CLEAT[1], side, principal_top_z, s)
...
print("Experiment 02 Fable v03: model generated")
