# Headless renderer for the Fable run of experiment 02.
#
# Usage:
...
# After rendering it runs a box-box (separating axis) overlap check on every
# pair of members and prints the penetrating pairs.
...
COLORS = {
    "Foundation": (0.75, 0.75, 0.75, 1.0),
    "Posts": (0.55, 0.36, 0.20, 1.0),
    "Bents": (0.68, 0.46, 0.25, 1.0),
    "Plates_and_Braces": (0.80, 0.58, 0.32, 1.0),
    "Purlins": (0.62, 0.42, 0.22, 1.0),
    "Common_Rafters": (0.85, 0.68, 0.42, 1.0),
    "Battens": (0.92, 0.80, 0.58, 1.0),
}
...
ROOF = ["Purlins", "Common_Rafters", "Battens"]

# View spec: azimuth/elevation in degrees, collections to hide, optional focus
# (center, radius) for close-ups.  Azimuth 0 = camera on +X looking at the
# east gable, 90 = camera on +Y looking at the north eave.
VIEWS = [
    dict(name="01", azim=35, elev=25, hide=[]),
    dict(name="02", azim=215, elev=25, hide=[]),
    dict(name="03", azim=0, elev=0, hide=ROOF),                       # bent elevation (gable)
    dict(name="04", azim=90, elev=0, hide=ROOF),                      # long elevation
    dict(name="05", azim=90, elev=89, hide=[]),                       # top
    dict(name="06", azim=35, elev=25, hide=ROOF),                     # frame only
    dict(name="07", azim=35, elev=25, hide=["Battens"]),              # purlins + commons
    dict(name="08", azim=0, elev=0, hide=ROOF, focus=(Vector((7.2, 0.0, 5.0)), 2.6)),   # truss close-up
    dict(name="09", azim=20, elev=15, hide=ROOF, focus=(Vector((7.2, 0.0, 4.6)), 1.4)),  # X crossing
    dict(name="10", azim=30, elev=20, hide=ROOF, focus=(Vector((7.2, -2.4, 3.4)), 1.2)), # eave joint
    dict(name="11", azim=20, elev=20, hide=[], focus=(Vector((7.2, 0.0, 7.1)), 1.2)),    # ridge
    dict(name="12", azim=120, elev=25, hide=[], focus=(Vector((3.6, 2.9, 3.9)), 1.6)),   # eave, roof layers
]
...
def obb(o):
    ...
    for i in range(3):
        ...
        half.append(col.length)           # unit cube has half-size 1
...
def penetration(a, b):
    ...
    for L in tests:
        if L.length < 1e-9:
            continue
        L = L.normalized()
        ra = sum(h * abs(L.dot(x)) for h, x in zip(ha, aa))
        rb = sum(h * abs(L.dot(x)) for h, x in zip(hb, ab))
        gap = abs(d.dot(L)) - ra - rb
        if gap >= 0:
            return 0.0
        depth = min(depth, -gap)
    return depth
