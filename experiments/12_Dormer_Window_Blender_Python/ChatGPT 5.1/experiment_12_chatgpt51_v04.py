# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 12 - CHATGPT 5.1 - V04
# DORMER WINDOW
# ------------------------------------------------------------------
...
def build_dormer_with_host_roof(
    ...
    z_bottom = z_main_ridge - main_tan * (back_y - y_main_ridge)
    bottom_L = Vector((x_left, back_y, z_bottom))
    bottom_R = Vector((x_right, back_y, z_bottom))

    # Provisional valley tops at main ridge (used only for intersection)
    top_L_prov = Vector((
    ...
    # ----------------------------------------------------------------
    # 1. Dormer support studs (as before: from floor to top plate)
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 4. Dormer ridge (extended upslope until it meets the large roof)
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 6. Gable studs in front/back (above header)
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 7. Main roof rafters (host roof) – aligned with dormer side studs
    # ----------------------------------------------------------------
    ...
    for i, x_pos in enumerate(x_positions):
        # Trim out rafters directly under the dormer opening
    ...
    # ----------------------------------------------------------------
    # 9. Double header at valley/ridge intersection (spanning dormer width)
    # ----------------------------------------------------------------
    header_center_x = (valley_top_L.x + valley_top_R.x) * 0.5
    header_y = y_ridge_back
    header_z = ridge_z
    header_len_x = abs(valley_top_R.x - valley_top_L.x)

    # Two plies (slightly offset in Y) to suggest a double header
    ...
    # ----------------------------------------------------------------
    # Placed at the main roof level under the dormer front.
    z_front_roof = z_main_ridge - main_tan * (front_y - y_main_ridge)
    ...
    # ----------------------------------------------------------------
    # 12. Vertical side studs between dormer top plates and dormer rafters
    #     (as before) – purely framing, not structural to main roof.
    # ----------------------------------------------------------------
    ...
    #     (shifted in Y so they do not intersect rafters)
    # ----------------------------------------------------------------
    ...
    for i, y_pos in enumerate(dormer_rafter_y_positions):
        # Shift halfway between this rafter line and the next towards back
        brace_y = y_pos - 0.5 * rafter_spacing
        box(
