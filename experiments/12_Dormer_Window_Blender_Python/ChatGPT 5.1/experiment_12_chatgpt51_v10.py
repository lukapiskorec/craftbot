# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 12 - CHATGPT 5.1 - V10
# DORMER WINDOW
# ------------------------------------------------------------------
...
def build_dormer_with_host_roof(
    ...
    # ----------------------------------------------------------------
    # 3. Dormer ridge (extended slightly upslope beyond roof)
    # ----------------------------------------------------------------
    ridge_extension_y = 2.0 * rafter_width  # <= two profile thicknesses
    ...
    # ----------------------------------------------------------------
    # 6. Double header at valley/ridge intersection (spanning across rafters)
    # ----------------------------------------------------------------
    ...
    # Valley tops meet the middle of the double header
    ...
    #    - Trimmed_RafterUpper_j run from header line to main ridge
    #    - Additional short rafters for j=0 and j=2 go from valley to header
    # ----------------------------------------------------------------
    if num_trimmed_center_rafters > 0:
        for j in range(num_trimmed_center_rafters):
            ...
            # upper trimmed piece: header line -> main ridge
            ...
            rafter_between(
                ...
                R=tuple(ridge_pt_vec),
                section_y=main_rafter_width,
                section_z=main_rafter_depth,
            )

            # second pair: short rafters from valley to header (for j=0 and j=2)
            if j == 0:
                # near left valley
                if abs(valley_top_L.x - bottom_L.x) > 1e-6:
                    t_val = (x_cut - bottom_L.x) / (valley_top_L.x - bottom_L.x)
                    t_val = max(0.0, min(1.0, t_val))
                    P_valley = bottom_L.lerp(valley_top_L, t_val)
                    rafter_between(
                        name="Trimmed_RafterUpper_LeftValley",
                        E=tuple(P_valley),
                        R=tuple(header_pt_vec),
                        section_y=main_rafter_width,
                        section_z=main_rafter_depth,
                    )
            elif j == 2:
                ...
                if abs(valley_top_R.x - bottom_R.x) > 1e-6:
                    t_val = (x_cut - bottom_R.x) / (valley_top_R.x - bottom_R.x)
                    t_val = max(0.0, min(1.0, t_val))
                    P_valley = bottom_R.lerp(valley_top_R, t_val)
                    rafter_between(
                        name="Trimmed_RafterUpper_RightValley",
                        E=tuple(P_valley),
                        R=tuple(header_pt_vec),
                        section_y=main_rafter_width,
                        section_z=main_rafter_depth,
    ...
    brace_width = 0.45 * clear_width  # scaled in X
