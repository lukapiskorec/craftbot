# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 12 - CHATGPT 5.1 - V08
# DORMER WINDOW
# ------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Helper: intersection of two 3D line segments projected in XY
# Returns (intersection_point, s_on_first) where:
#   point = p1 + s*(p2 - p1)
# -------------------------------------------------------------------
def intersect_segments_xy(p1, p2, p3, p4):
    x1, y1 = p1.x, p1.y
    x2, y2 = p2.x, p2.y
    x3, y3 = p3.x, p3.y
    x4, y4 = p4.x, p4.y
    ...
    if abs(denom) < 1e-8:
        # Parallel or nearly parallel – just return midpoint of first segment
        return (p1 + p2) * 0.5, 0.5

    s = ((x3 - x1) * dy2 - (y3 - y1) * dx2) / denom
    point = p1 + (p2 - p1) * s
    return point, s
...
def build_dormer_with_host_roof(
        ...
        num_dormer_jacks=3,
        # trimmed centre rafters inside dormer
    ...
    # Y position where main roof plane is at height ridge_z
    # (line of the double header and valley tops in Y)
    y_ridge_back = y_main_ridge - (ridge_z - z_main_ridge) / main_tan
    z_header_roof = ridge_z

    # Valley feet at back of dormer
    bottom_L = Vector((x_left, back_y, z_back_roof))
    bottom_R = Vector((x_right, back_y, z_back_roof))

    # ----------------------------------------------------------------
    # 1. Front window framing (no back studs)
    # ----------------------------------------------------------------
    ...
    for i, x_pos in enumerate(x_positions):
        ...
        if x_left < x_pos < x_right:
            ...
            # upper part: from double-header line (y_ridge_back) -> main ridge
    ...
    # ----------------------------------------------------------------
    # 6. Double header line in X (we need x-span before valleys)
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 8. EXTRA trimmed centre rafters inside dormer opening
    #    j=0 and j=2 are cut against the valley rafters.
    # ----------------------------------------------------------------
    if num_trimmed_center_rafters > 0:
        for j in range(num_trimmed_center_rafters):
            ...
            # upper piece: from header line to either ridge or valley
            header_pt_vec = Vector((x_cut, y_ridge_back, z_header_roof))
            ridge_pt_vec = Vector((x_cut, y_main_ridge, z_main_ridge))

            if j == 0:
                # intersect with left valley
                inter_pt, _ = intersect_segments_xy(
                    header_pt_vec, ridge_pt_vec,
                    bottom_L, valley_top_L
                )
                end_vec = inter_pt
            elif j == 2:
                # intersect with right valley
                inter_pt, _ = intersect_segments_xy(
                    header_pt_vec, ridge_pt_vec,
                    bottom_R, valley_top_R
                )
                end_vec = inter_pt
            else:
                # middle one goes full to the ridge
                end_vec = ridge_pt_vec

            rafter_between(
                name=f"Trimmed_RafterUpper_{j}",
                E=tuple(header_pt_vec),
                R=tuple(end_vec),
                section_y=main_rafter_width,
                section_z=main_rafter_depth,
            )
    ...
    # ----------------------------------------------------------------
    # 9. Jack rafters between extended Dormer_Ridge and Valley_Rafters
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 10. Lower front support beam for the dormer
    # ----------------------------------------------------------------
    box(
    ...
    # ----------------------------------------------------------------
    # 11. Vertical side studs from dormer rafters down to main roof rafters
    # ----------------------------------------------------------------
    ...
    for i, y_pos in enumerate(dormer_rafter_y_positions):
        ...
        # Bottom is top of main roof rafter (roof plane) at this y
    ...
    # ----------------------------------------------------------------
    # 12. Horizontal top braces tying dormer rafters
    #     Shifted in -Y and scaled by 0.45 in X so they sit well inside.
    # ----------------------------------------------------------------
    brace_thickness = plate_thickness
    z_brace = ridge_z - (ridge_z - top_plate_z) * 0.3

    # original clear width between inner faces of side studs
    clear_width = width - 2.0 * stud_width
    brace_width = 0.45 * clear_width  # requested 0.45 scale in X
