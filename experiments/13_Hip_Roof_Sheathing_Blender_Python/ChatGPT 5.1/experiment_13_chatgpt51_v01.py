# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 13 - CHATGPT 5.1 - V01
...
def create_board_between(name, p0, p1, normal, board_width, board_thickness):
    ...
    # Ensure normal is not parallel to length
    if abs(axis_y.dot(axis_z)) > 0.999:
        # Fallback: use global up as normal-ish
        axis_y = Vector((0.0, 0.0, 1.0))
        if abs(axis_y.dot(axis_z)) > 0.999:
            axis_y = Vector((0.0, 1.0, 0.0))

    axis_x = axis_y.cross(axis_z).normalized()
    axis_y = axis_z.cross(axis_x).normalized()  # enforce orthonormal basis
...
def cover_quad_with_boards(
    ...
    Returns the next available index (start_index + number_of_boards).
    """
    P00 = Vector(P00)
    P10 = Vector(P10)
    P11 = Vector(P11)
    ...
    # Lengths along v at u=0 and u=1
    v_edge0 = P01 - P00
    v_edge1 = P11 - P10
    ...
    # Number of boards along v
    N = max(1, int(math.ceil(avg_len_v / board_width)))
    dv = 1.0 / N
    # Actual width so boards exactly span the average length
    ...
    for i in range(N):
        ...
        # Points on the two "side" edges (u = 0 and u = 1)
...
# ---------------------------------------------------------------------------
# MAIN HIP ROOF SHEATHING (NORTH AND SOUTH SLOPES)
# ---------------------------------------------------------------------------
...
def add_main_roof_board_sheathing(
    ...
    """
    Generate closed wood-board sheathing on the main hip roof
    (north and south trapezoidal slopes), based on main_geom
    returned by build_main_hip_roof().
    """
    ...
    # North slope quad: RW (u=0,v=0), RE (u=1,v=0), NE (u=1,v=1), NW (u=0,v=1)
    ...
    # South slope quad: RE (u=0,v=0), RW (u=1,v=0), SW (u=1,v=1), SE (u=0,v=1)
    index = cover_quad_with_boards(
    ...
    # NOTE:
    # This covers the two main trapezoidal slopes. The small triangular
    # hip faces at east and west ends (between NE/SE and ridge_E, and
    # between NW/SW and ridge_W) can be covered using the same pattern
    # by defining appropriate triangular/degenerate quad patches.


# ---------------------------------------------------------------------------
# DORMER ROOF SHEATHING (LEFT AND RIGHT HALF-ROOFS)
# ---------------------------------------------------------------------------
...
def add_dormer_board_sheathing(
    ...
    """
    Generate wood-board sheathing over the left and right half-roofs
    of a dormer on the north slope, using the same scalar geometry
    as build_dormer_on_north_slope(), but only creating boards.

    This approximates the dormer roof planes as two quads:
    left:  eave-left line x -> ridge line at center x
    right: eave-right line x -> ridge line at center x
    each from y_ridge_back (where it dives into main roof) to front_y.
    """
    ...
    # Eave positions (outer edge of board sheathing at dormer)
    x_eave_L = ox - half_w - overhang
    x_eave_R = ox + half_w + overhang

    # Define quads for left and right dormer planes
    # Left quad:
    #   P00_L: back, eave-left
    #   P10_L: front, eave-left
    #   P11_L: front, ridge center
    #   P01_L: back, ridge center
    ...
    # Cover left dormer half-roof with boards (boards along Y)
    idx = 0
...
def build_scene_with_sheathing():
    """
    Example scene assembly that builds the existing hip roof, T-wing,
    dormers, and then adds board sheathing on the main roof and dormer roofs.
    """

    # Main hip roof parameters (same as your current build_scene) :contentReference[oaicite:31]{index=31}
    length = 18.0
    width = 9.0
    plate_z = 0.0
    ...
    for d_id, cx in enumerate(center_offsets):
        ...
        # Add dormer roof sheathing for this dormer
    ...
    # Restore gap rafters between dormers
    gap_xs = []
