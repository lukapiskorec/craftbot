# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 13 - FABLE - V04
...
def build_scene():
    ...
    # ridge boards: square-edged, so their top corners must clear both slopes
    drops["Ridge_Board"] = drop_member("Ridge_Board", [F["N"], F["S"]])
    drops["Wing_Ridge"] = drop_member("Wing_Ridge", [F["WE"], F["WW"]])
    for d_id in range(len(dormers)):
        name = f"D{d_id}_Dormer_Ridge"
        drops[name] = drop_member(name, [F[f"D{d_id}L"], F[f"D{d_id}R"]])
