# ------------------------------------------------------------------
# CRAFT BOT - EXPERIMENT 13 - FABLE - V01
        ...
        for j in range(n_jacks):
            ...
            if side == "R":
                ...
                base.box(
                    ...
                    center=(0.5 * (P_L.x + P_valley.x), y_j - dg["stud_depth"],
                            P_L.z + 0.6 * (ridge_z - P_L.z)),
                    size=(abs(P_valley.x - P_L.x) * 0.48, dg["stud_depth"], dg["plate_thickness"]),
                )
...
def build_scene():
    ...
    for d_id, dg in enumerate(dormers):
        ...
        xl, xr = dg["x_left"] - dg["stud_width"] * 0.5, dg["x_right"] + dg["stud_width"] * 0.5
        eb_L = P(L, ct["N"], vplane("x", dg["ox"] - dg["run"]))
        eb_R = P(R, ct["N"], vplane("x", dg["ox"] + dg["run"]))
        apex = P(L, R, ct["N"])
        y_f, y_eb = vplane("y", dg["front_y"]), vplane("y", eb_L.y)
        holes_N.append([
            P(c0["N"], vplane("x", xl), y_f), P(c0["N"], vplane("x", xr), y_f),
            P(c0["N"], vplane("x", xr), y_eb), eb_R, apex, eb_L,
            P(c0["N"], vplane("x", xl), y_eb),
        ])
